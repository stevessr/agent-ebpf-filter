...
3. 运行 Research security evaluation，对比预期标签与实际风险/决策。
4. 导出 bundle，产物包含：
   - `events.jsonl/csv`
   - `training.jsonl/csv`
...
`training-manifest.json` 会记录 schema version、label policy、feature space/version、redaction level 分布、label/category/source 分布、normalization 与 quality，便于复现实验。
