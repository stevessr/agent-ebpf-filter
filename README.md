...
- **Kernel ML / DKMS**: the optional `kernel-ml` DKMS module now exposes `kernel` / `cuda` / `auto` inference backends; CUDA runs through a userspace `kernel_ml_cuda_helper` and falls back to the in-kernel fixed-point path on helper/GPU timeout.
