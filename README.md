...
- **Plugins**: register custom eBPF plugins from templates, raw C source, or a low-code multi-block visual builder whose primary workspace is a Dify-style node workflow with a searchable node-type library, draggable Trigger / Condition / Map / Action / Code nodes, drag-to-connect ports, editable wire connections with disconnected-flow compile gates, undo/redo workspace history, animated wires, node-level inspectors for quick block editing, and a separate second-level Map / Blueprint tab for detailed condition-tree, state-map, action, and metadata editing, plus scenario recipes, browser draft autosave/restore, JSON import/export, compile-readiness validation, generated C preview, compile, and load controls
