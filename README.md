...
- **Kernel ML / DKMS**: the optional `kernel-ml` DKMS module now exposes `kernel` / `cuda` / `auto` inference backends, sysfs controls under `/sys/kernel/kernel_ml`, v2 model metadata (`model_generation`, dynamic tree count/depth, up to 16 classes), exact-match LRU inference caching, perf/Nsight profiling helpers, and a userspace CUDA `kernel_ml_cuda_helper` with fallback to the in-kernel fixed-point path on helper/GPU timeout.
...
`libcudart` / `libcuda`.
For the separate `kernel-ml` DKMS module, CUDA support is intentionally split:
the module builds without CUDA, while `make -C kernel-ml cuda-helper` builds
the userspace GPU helper when the CUDA toolkit is available.
