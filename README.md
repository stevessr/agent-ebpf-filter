...
- **Plugins**: register custom eBPF plugins from templates, raw C source, or a low-code multi-block visual builder whose primary workspace is a Dify-style node workflow with a searchable node-type library, draggable/deletable Trigger / Condition / Map / Action / Code nodes, a manually resizable canvas, drag-to-connect ports, editable wire connections with disconnected-flow compile gates, undo/redo workspace history, animated wires, node-level inspectors for quick block editing, a dockable/hideable floating scenario-block recipe window, a separate NLP Blocks Compiler tab for natural-language-to-block generation, a separate Map / Blueprint tab for detailed condition-tree, state-map, action, and metadata editing, and a separate Generated eBPF C tab for generated source, compile logs, and load controls, plus browser draft autosave/restore, JSON import/export, and compile-readiness validation
...
- The System Health tab shows collector health, including ringbuf reserve-fail counters and per-event-type totals.
