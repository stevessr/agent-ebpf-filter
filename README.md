...
- **Plugins**: register custom eBPF plugins from templates, raw C source, or a low-code multi-block visual builder with a Dify-style searchable node-type library, draggable Trigger / Condition / Map / Action / Code nodes, drag-to-connect ports, editable wire connections with disconnected-flow compile gates, undo/redo workspace history, animated wires, node-level inspectors for quick block editing, scenario recipes, browser draft autosave/restore, JSON import/export, compile-readiness validation, generated C preview, compile, and load controls
