...
func init() {
	if ret := nvml.Init(); ret == nvml.SUCCESS {
		...
		log.Printf("NVML Init failed: %v. Falling back to nvidia-smi if needed.", nvml.ErrorString(ret))
	}
}
...
func getGPUMetrics() (map[int32]gpuInfo, []*pb.GPUStatus) {
	...
	if !nvmlInitialized {
		// Fallback logic could go here if we wanted to maintain nvidia-smi parsing,
		// but using NVML is preferred as requested.
	...
	for i := 0; i < count; i++ {
		...
		globalStats = append(globalStats, &pb.GPUStatus{
			...
			UtilGpu:  util.Gpu,
			UtilMem:  util.Memory,
			MemTotal: uint32(memInfo.Total / 1024 / 1024),
			MemUsed:  uint32(memInfo.Used / 1024 / 1024),
			Temp:     temp,
		})
	...
	api := r.Group("/", authMiddleware())
	{
		config := api.Group("/config")
		{
			...
			config.POST("/comms", func(c *gin.Context) {
				var req struct{ Comm, Tag string `json:"comm" json:"tag"` }
				_ = c.ShouldBindJSON(&req)
				var k [16]byte
				copy(k[:], req.Comm)
				_ = objs.TrackedComms.Put(k, getTagID(req.Tag))
				c.JSON(200, gin.H{"status": "ok"})
			})
			...
			config.POST("/paths", func(c *gin.Context) {
				var r struct{ Path, Tag string `json:"path" json:"tag"` }
