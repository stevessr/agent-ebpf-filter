...
func getCoreTypes() []pb.CPUInfo_Core_Type {
	...
	for i := 0; i < cores; i++ {
		...
		if err == nil {
			...
			if val == "intel_atom" { types[i] = pb.CPUInfo_Core_EFFICIENCY; continue }
			if val == "intel_core" { types[i] = pb.CPUInfo_Core_PERFORMANCE; continue }
		}
		...
		if err == nil {
			...
			if maxFreqs[i] > overallMax { overallMax = maxFreqs[i] }
		}
	}
	if overallMax > 0 {
		for i := 0; i < cores; i++ {
			if types[i] != 0 { continue }
			if maxFreqs[i] < (overallMax * 8 / 10) { types[i] = pb.CPUInfo_Core_EFFICIENCY } else { types[i] = pb.CPUInfo_Core_PERFORMANCE }
		}
	}
	return types
}
...
func startUDSServer(broadcast chan *pb.Event) {
	_ = os.Remove(udsPath)
	l, err := net.Listen("unix", udsPath); if err != nil { return }
	_ = os.Chmod(udsPath, 0666); defer l.Close()
	for {
...
func main() {
	if os.Geteuid() != 0 {
		...
		sudoCmd := "sudo"; if isDesktop { if p, err := exec.LookPath("pkexec"); err == nil { sudoCmd = p } }
	...
	_ = bpf.LoadAgentTrackerObjects(&objs, nil); defer objs.Close()

	attach := func(c, n string, p *ebpf.Program) link.Link {
		l, err := link.Tracepoint(c, n, p, nil); if err != nil { log.Printf("Link %s/%s: %v", c, n, err) }; return l
	}
	links := []link.Link{
	...
	go func() {
		for event := range broadcast {
			data, _ := proto.Marshal(event); clientsMu.Lock()
			for c := range clients { if err := c.WriteMessage(websocket.BinaryMessage, data); err != nil { c.Close(); delete(clients, c) } }
			clientsMu.Unlock()
		}
	}()
	...
	r.GET("/ws/system", func(c *gin.Context) {
		conn, err := upgrader.Upgrade(c.Writer, c.Request, nil); if err != nil { return }; defer conn.Close()
		...
		for range ticker.C {
			gm, gs := getGPUMetrics()
			vm, _ := mem.VirtualMemory(); cc, _ := cpu.Percent(0, false); cp, _ := cpu.Percent(0, true)
			netIO, _ := gnet.IOCounters(true); diskIO, _ := disk.IOCounters()
			pbIO := &pb.IOInfo{}
			for _, n := range netIO {
			...
			stats := &pb.SystemStats{Gpus: gs, Cpu: cpuInfo, Memory: &pb.MemoryInfo{Total: vm.Total, Used: vm.Used, Percent: float32(vm.UsedPercent)}, Io: pbIO}
			psList, _ := ps.Processes()
			for _, p := range psList {
				n, _ := p.Name(); pp, _ := p.Ppid(); ccp, _ := p.CPUPercent(); mp, _ := p.MemoryPercent(); u, _ := p.Username(); cmdl, _ := p.Cmdline(); ct, _ := p.CreateTime()
				gmem, gid, gutil := uint32(0), uint32(0), uint32(0)
				if info, ok := gm[p.Pid]; ok { gmem, gid, gutil = info.mem, info.gpu, info.util }
				stats.Processes = append(stats.Processes, &pb.Process{Pid: p.Pid, Ppid: pp, Name: n, Cpu: ccp, Mem: mp, User: u, GpuMem: gmem, GpuId: gid, GpuUtil: gutil, Cmdline: cmdl, CreateTime: ct})
			}
	...
	api := r.Group("/", authMiddleware())
	{
		config := api.Group("/config")
		{
			...
			config.POST("/tags", func(c *gin.Context) {
				var r struct{ Name string `json:"name"` }; _ = c.ShouldBindJSON(&r); getTagID(r.Name); c.JSON(200, gin.H{"status": "ok"})
			})
			...
			config.POST("/comms", func(c *gin.Context) {
				var r struct{ Comm, Tag string `json:"comm" json:"tag"` }; _ = c.ShouldBindJSON(&r); var k [16]byte; copy(k[:], r.Comm); _ = objs.TrackedComms.Put(k, getTagID(r.Tag)); c.JSON(200, gin.H{"status": "ok"})
			})
			...
			config.POST("/paths", func(c *gin.Context) {
				var r struct{ Path, Tag string `json:"path" json:"tag"` }; _ = c.ShouldBindJSON(&r); var k [256]byte; copy(k[:], r.Path); _ = objs.TrackedPaths.Put(k, getTagID(r.Tag)); c.JSON(200, gin.H{"status": "ok"})
			})
		...
		system := api.Group("/system")
		{
			system.GET("/ls", func(c *gin.Context) {
				...
				for _, v := range e { fp := p; if fp == "/" { fp = "/" + v.Name() } else { fp = fp + "/" + v.Name() }; l = append(l, gin.H{"name": v.Name(), "isDir": v.IsDir(), "path": fp}) }
				c.JSON(200, l)
			})
			system.POST("/run", func(c *gin.Context) {
				...
				if err := c.ShouldBindJSON(&r); err == nil {
					...
					cmd := exec.Command(wb, append([]string{r.Comm}, r.Args...)...); cmd.Env = os.Environ()
	...
	staticDir := "../frontend/dist"; if _, err := os.Stat(staticDir); err != nil { staticDir = "./frontend/dist" }
	r.StaticFile("/", filepath.Join(staticDir, "index.html")); r.Static("/assets", filepath.Join(staticDir, "assets")); r.NoRoute(func(c *gin.Context) { c.File(filepath.Join(staticDir, "index.html")) })
