...
func main() {
	...
	for _, imp := range improvements {
		...
		fmt.Printf("%-30s | %7.1f%% | %7.1f%% | %5.1f%% | %s\n",
			imp.Dataset, imp.Base, imp.Attn, imp.Imp, imp.Status)
	}

	fmt.Println(strings.Repeat("-", 80))
	fmt.Printf("平均提升：%.2f%% | 有效提升案例：%d/%d\n",
		totalImp/float64(len(improvements)), count, len(improvements))
...
func printTable(results []struct {
	...
	for _, r := range results {
		...
		fmt.Printf("%-40s | %7.1f%% | %8.1f%% | %6.1f%% | %7.1f%%%s\n",
			r.Model, r.Acc, r.Prec, r.Rec, r.F1, marker)
	}
}
