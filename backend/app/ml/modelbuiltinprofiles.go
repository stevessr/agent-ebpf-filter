...
var BuiltinModelProfiles = []builtinModelProfile{
	...
	p(ModelGANTransformer, ModelGANTransformer, "GAN + Transformer", "生成式深度模型", "Class-conditioned GAN 数据扩增 + Transformer 编码判别器，适合少样本与边界样本增强对照。", false, map[string]int{"numTrees": 16, "maxDepth": 6, "minSamplesLeaf": 4}, []string{"gan", "transformer", "synthetic", "deep"}, nil),
