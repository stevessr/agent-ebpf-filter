...
func TestOSFrontendSecuritySurfaceWiresSandboxEndpoints(t *testing.T) {
	...
	componentPaths := []string{
		filepath.Join("..", "..", "frontend", "src", "components", "config", "ConfigSecurityTab.vue"),
		filepath.Join("..", "..", "frontend", "src", "components", "config", "security", "ConfigCgroupSandboxCard.vue"),
		filepath.Join("..", "..", "frontend", "src", "components", "config", "security", "ConfigLsmEnforcerCard.vue"),
	}
	...
	for _, want := range []string{
		...
		if !strings.Contains(component, want) {
			t.Fatalf("security components missing UI sandbox control %q", want)
		}
	}
