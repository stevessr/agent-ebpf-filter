...
// TracepointBootstrapStatus is the JSON-serializable status reported by /system/bootstrap-health.
...
// RecordTracepointBootstrapStatus stores the bootstrap status.
...
// SnapshotBootstrapTracepointStatus returns a copy of the current status.
func SnapshotBootstrapTracepointStatus() TracepointBootstrapStatus {
	bootstrapTracepointStatusStore.mu.RLock()
	defer bootstrapTracepointStatusStore.mu.RUnlock()
...
// HandleBootstrapHealth is the HTTP handler for /system/bootstrap-health.
func HandleBootstrapHealth(c *gin.Context) {
	c.JSON(200, SnapshotBootstrapTracepointStatus())
}
