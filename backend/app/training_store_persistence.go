...
func persistTrainingStoreSnapshot(dataDir, persistPath string, samples []TrainingSample) error {
	...
	file, tempName, err := createRecordingTemp(root, "ml-training-data")
	if err != nil {
	...
	if err := replaceRecordingDestination(root, tempName, targetName); err != nil {
...
func readTrainingStoreFile(persistPath string, maxSamples int) ([]TrainingSample, error) {
	...
	file, err := openRecordingChild(root, targetName, unix.O_RDONLY, 0)
...
func decodeTrainingStoreArgs(payload []byte) ([]string, error) {
	...
	if strings.HasPrefix(raw, "[") && strings.HasSuffix(raw, "]") {
		...
		return splitCommandLine(fallback), nil
	}
