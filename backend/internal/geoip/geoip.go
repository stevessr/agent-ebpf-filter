...
type Resolver struct {
	mu       sync.RWMutex
	cache    map[string]*CacheEntry // IP -> record
	maxCache int
	ttl      time.Duration
	hits     uint64
	misses   uint64
...
func (r *Resolver) Lookup(ipStr string) (Record, bool) {
	if r == nil {
		return Record{}, false
	}
...
func (r *Resolver) resolveIP(ip net.IP) Record {
...
func classifyIPToRegion(ip net.IP) Record {
	InitDatabase()
...
func EnrichEndpointWithGeoIP(resolver *Resolver, endpoint string) string {
	host := endpoint
	...
	record, ok := resolver.Lookup(host)
