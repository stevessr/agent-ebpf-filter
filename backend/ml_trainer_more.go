...
func (t *ModelTrainer) trainNaiveBayes(store *TrainingDataStore, cfg MLConfig) (Model, TrainResult) {
	t.acquire(); defer t.release()
	t.ResetCancel(); t.isRunning = true; t.progress = 0
	...
	for _, s := range labeled {
		if s.Label < 0 || int(s.Label) >= m.Classes { continue }
		c := s.Label
		counts[c]++
...
func (t *ModelTrainer) trainExtraTrees(store *TrainingDataStore, cfg MLConfig) (Model, TrainResult) {
	...
	for i, s := range labeled { samples[i] = trainSample{features: s.Features, label: s.Label} }
	trainSet, valSet, _, _, _ := prepareAutoTuneSplit(labeled, cfg.ValidationSplitRatio)

	nt := cfg.NumTrees; if nt < 1 { nt = 31 }
	md := cfg.MaxDepth; if md < 1 { md = 8 }
	ml := cfg.MinSamplesLeaf; if ml < 1 { ml = 5 }
...
func (t *ModelTrainer) trainAdaBoost(store *TrainingDataStore, cfg MLConfig) (Model, TrainResult) {
	...
	nEst := cfg.NumTrees; if nEst < 10 { nEst = 50 }
	m := NewAdaBoost(nEst)
	...
	for e := 0; e < nEst; e++ {
		...
		if t.IsCancelled() { return nil, TrainResult{Error: "cancelled"} }
		...
		for tries := 0; tries < 50; tries++ {
			...
			for i, s := range samples {
				if s.features[fi] < thresh {
					if s.label != 1 { leftErr += weights[i] }
					leftW += weights[i]
				} else {
					if s.label != 0 { rightErr += weights[i] }
					rightW += weights[i]
				}
			}
		...
		for i, s := range samples {
			...
			if pred != int(s.label) { weights[i] *= math.Exp(alpha) }
		}
...
func (t *ModelTrainer) trainSVM(store *TrainingDataStore, cfg MLConfig) (Model, TrainResult) {
	...
	lr := 0.01; if cfg.NumTrees > 0 { lr = float64(cfg.NumTrees) / 1000.0 }
	maxIter := cfg.MinSamplesLeaf; if maxIter < 100 { maxIter = 1000 }
...
func (t *ModelTrainer) trainRidge(store *TrainingDataStore, cfg MLConfig) (Model, TrainResult) {
	...
	alpha := 1.0; if cfg.NumTrees > 0 { alpha = float64(cfg.NumTrees) / 100.0 }
...
func (t *ModelTrainer) trainPA(store *TrainingDataStore, cfg MLConfig) (Model, TrainResult) {
	...
	C := 1.0; if cfg.NumTrees > 0 { C = float64(cfg.NumTrees) / 10.0 }
	maxIter := cfg.MinSamplesLeaf; if maxIter < 100 { maxIter = 1000 }
...
func trainSGD(W [][FeatureDim + 1]float64, nClasses int, labeled []TrainingSample, lr float64, maxIter int, C float64, loss string, t *ModelTrainer) {
	...
	for iter := 0; iter < maxIter; iter++ {
		...
		for _, idx := range order {
			s := labeled[idx]
			if s.Label < 0 || int(s.Label) >= nClasses { continue }

			scores := make([]float64, nClasses)
			for c := 0; c < nClasses; c++ {
				scores[c] = W[c][FeatureDim]
				for d := 0; d < FeatureDim; d++ { scores[c] += W[c][d] * s.Features[d] }
			}
			...
			case "hinge": // SVM
				if margin < 1.0 {
					for d := 0; d <= FeatureDim; d++ {
						vT, vW := 0.0, 0.0
						if d == FeatureDim { vT, vW = 1.0, 1.0 } else { vT, vW = s.Features[d], s.Features[d] }
			...
			case "perceptron":
				if margin <= 0 {
					for d := 0; d <= FeatureDim; d++ {
						v := 0.0; if d == FeatureDim { v = 1.0 } else { v = s.Features[d] }
						W[trueC][d] += eta * v
						W[wrongC][d] -= eta * v
					}
				}
			case "pa":
				if margin < 1.0 {
					normSq := 0.0
					for d := 0; d < FeatureDim; d++ { normSq += s.Features[d] * s.Features[d] }
					normSq += 1.0 // bias
...
func ridgeFit(W [][FeatureDim + 1]float64, nClasses int, labeled []TrainingSample, alpha float64) {
	...
	for c := 0; c < nClasses; c++ {
		for iter := 0; iter < 500; iter++ {
			...
			for _, s := range labeled {
				target := 0.0
				if int(s.Label) == c { target = 1.0 }
				dot := W[c][FeatureDim]
				for d := 0; d < FeatureDim; d++ { dot += W[c][d] * s.Features[d] }
				err := dot - target
				for d := 0; d < FeatureDim; d++ {
...
func evalModelLabeled(model Model, labeled []TrainingSample) float64 {
	if len(labeled) == 0 { return 0 }
	correct := 0
	for _, s := range labeled {
		if model.Predict(s.Features).Action == s.Label { correct++ }
	}
...
func evalLinearModel(W [][FeatureDim + 1]float64, nClasses int, samples []trainSample) float64 {
	...
	for _, s := range samples {
		...
		for c := 0; c < nClasses; c++ {
			...
			for d := 0; d < FeatureDim; d++ { score += W[c][d] * s.features[d] }
			if score > bestS { bestS = score; bestC = c }
		}
		if int32(bestC) == s.label { correct++ }
	}
	return float64(correct) / float64(len(samples))
}
...
func toTrainSamples(labeled []TrainingSample) []trainSample {
	...
	for i, s := range labeled { out[i] = trainSample{features: s.Features, label: s.Label} }
	return out
}
