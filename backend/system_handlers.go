...
func handleSystemLs(c *gin.Context) {
	...
	for _, v := range e {
		if !showHidden && strings.HasPrefix(v.Name(), ".") { continue }
	...
	if limit > 0 && start+limit < total { end = start + limit }
	...
	for _, v := range slice {
		...
		if err == nil {
			...
			if !v.IsDir() { mType = mime.TypeByExtension(filepath.Ext(v.Name())) }
		}
...
func handleFilePreview(c *gin.Context) {
	...
	if err != nil {
		if os.IsNotExist(err) { c.JSON(404, gin.H{"error": "path not found"}); return }
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
	c.JSON(200, preview)
...
func handleDownload(c *gin.Context) {
	p := c.Query("path")
	if p == "" { c.JSON(400, gin.H{"error": "path is required"}); return }
	info, err := os.Stat(p)
	if err != nil || info.IsDir() { c.JSON(404, gin.H{"error": "file not found"}); return }
	c.File(p)
}

func handleUpload(c *gin.Context) {
	dir := c.Query("path")
	if dir == "" { dir = getRealHomeDir() }
	file, err := c.FormFile("file")
	if err != nil { c.JSON(400, gin.H{"error": "no file uploaded"}); return }
...
func handleSystemdServices(c *gin.Context) {
	...
	if scope == "user" { args = append([]string{"--user"}, args...) }
	...
	if err != nil { c.JSON(500, gin.H{"error": fmt.Sprintf("%v: %s", err, string(out))}); return }
...
func handleSystemdControl(c *gin.Context) {
	var req struct { Unit string `json:"unit"`; Action string `json:"action"`; Scope string `json:"scope"` }
	if err := c.ShouldBindJSON(&req); err != nil { c.JSON(400, gin.H{"error": "invalid request"}); return }
	validActions := map[string]bool{"start": true, "stop": true, "restart": true}
	if !validActions[req.Action] { c.JSON(400, gin.H{"error": "invalid action"}); return }

	args := []string{req.Action, req.Unit}
	if req.Scope == "user" {
		args = append([]string{"--user"}, args...)
		cmd := exec.Command("systemctl", args...)
		if err := cmd.Run(); err != nil { c.JSON(500, gin.H{"error": err.Error()}); return }
	} else {
...
func handleSystemdLogs(c *gin.Context) {
	unit := c.Query("unit"); lines := c.DefaultQuery("lines", "100"); scope := c.DefaultQuery("scope", "system")
	if unit == "" { c.JSON(400, gin.H{"error": "unit is required"}); return }
...
func handleTrackedComms(c *gin.Context) {
	...
	for iter.Next(&k, &tid) { items = append(items, string(bytes.TrimRight(k[:], "\x00"))) }
	c.JSON(200, items)
}

func handleProcessSignal(c *gin.Context) {
	var req struct { PID int `json:"pid"`; Signal string `json:"signal"` }
	if err := c.ShouldBindJSON(&req); err != nil { c.JSON(400, gin.H{"error": "invalid request"}); return }
	p, err := os.FindProcess(req.PID)
	if err != nil { c.JSON(404, gin.H{"error": "process not found"}); return }
	var sig os.Signal
	switch strings.ToLower(req.Signal) {
	case "stop": sig = syscall.SIGSTOP
	case "cont": sig = syscall.SIGCONT
	case "kill": sig = syscall.SIGKILL
	case "term": sig = syscall.SIGTERM
	default: c.JSON(400, gin.H{"error": "unsupported signal"}); return
	}
	if err := p.Signal(sig); err != nil { c.JSON(500, gin.H{"error": err.Error()}); return }
	c.JSON(200, gin.H{"status": "ok"})
}
...
func handleMicrophones(c *gin.Context) {
	...
	for _, m := range matches {
		if len(m) >= 5 { devices = append(devices, gin.H{"id": fmt.Sprintf("hw:%s,%s", m[1], m[3]), "name": fmt.Sprintf("%s (%s)", m[2], m[4])}) }
	}
	if len(devices) == 0 { devices = append(devices, gin.H{"id": "default", "name": "Default Input"}) }
	c.JSON(200, devices)
}
...
func serveMicrophoneWS(c *gin.Context) {
	...
	go func() {
		select { case <-done: case <-c.Request.Context().Done(): }
		_ = cmd.Process.Kill()
	}()
	...
	for {
		...
		if err := conn.WriteMessage(websocket.BinaryMessage, buf[:n]); err != nil { break }
	}
	_ = cmd.Wait()
}
...
func serveSensorsWS(c *gin.Context) {
	...
	for {
		...
		case <-ticker.C:
			temps, _ := host.SensorsTemperatures()
			if err := conn.WriteJSON(gin.H{"temperatures": temps, "fans": []string{}}); err != nil { return }
		case <-done: return
		}
	}
}
...
func serveSystemStatsWS(c *gin.Context) {
	...
	if err != nil { lastFaults = vmFaultCounters{} }
	lastFaultTime := time.Now()
	type procCPUSample struct { createTime int64; totalCPU float64; sampleTime time.Time }
	procCPUSamples := make(map[int32]procCPUSample)
	cpuScale := float64(runtime.NumCPU()); if cpuScale <= 0 { cpuScale = 1 }
	for range ticker.C {
		now := time.Now(); gm, gs := getGPUMetrics(); vm, _ := mem.VirtualMemory(); cc, _ := cpu.Percent(0, false); cp, _ := cpu.Percent(0, true); netIO, _ := gnet.IOCounters(true); diskIO, _ := disk.IOCounters()
		pbIO := &pb.IOInfo{}; vmFaults, faultErr := readVMFaultCounters(); faultInfo := &pb.FaultInfo{}; currentPIDs := make(map[int32]struct{})
		if faultErr == nil {
			pageFaults := vmFaults.pageFaults; majorFaults := vmFaults.majorFaults; minorFaults := uint64(0)
			if pageFaults >= majorFaults { minorFaults = pageFaults - majorFaults }
			faultInfo.PageFaults = pageFaults; faultInfo.MajorFaults = majorFaults; faultInfo.MinorFaults = minorFaults
			dt := now.Sub(lastFaultTime).Seconds()
			if dt > 0 {
				pageDelta := deltaUint64(pageFaults, lastFaults.pageFaults); majorDelta := deltaUint64(majorFaults, lastFaults.majorFaults); swapInDelta := deltaUint64(vmFaults.swapIn, lastFaults.swapIn); swapOutDelta := deltaUint64(vmFaults.swapOut, lastFaults.swapOut)
				faultInfo.PageFaultRate = float64(pageDelta) / dt; faultInfo.MajorFaultRate = float64(majorDelta) / dt
				faultInfo.MinorFaultRate = faultInfo.PageFaultRate - faultInfo.MajorFaultRate; if faultInfo.MinorFaultRate < 0 { faultInfo.MinorFaultRate = 0 }
				faultInfo.SwapIn = vmFaults.swapIn; faultInfo.SwapOut = vmFaults.swapOut; faultInfo.SwapInRate = float64(swapInDelta) / dt; faultInfo.SwapOutRate = float64(swapOutDelta) / dt
			}
			lastFaults = vmFaults; lastFaultTime = now
		}

		for _, n := range netIO {
			pbIO.Networks = append(pbIO.Networks, &pb.NetworkInterface{Name: n.Name, RecvBytes: n.BytesRecv, SentBytes: n.BytesSent})
			pbIO.TotalNetRecvBytes += n.BytesRecv; pbIO.TotalNetSentBytes += n.BytesSent
		}
		for name, d := range diskIO {
			...
			pbIO.TotalReadBytes += d.ReadBytes; pbIO.TotalWriteBytes += d.WriteBytes
		}
		cpuInfo := &pb.CPUInfo{Total: cc[0], Cores: cp}
		for i, usage := range cp {
			ct := pb.CPUInfo_Core_PERFORMANCE; if i < len(coreTypes) { ct = coreTypes[i] }
		...
		for _, p := range psList {
			n, _ := p.Name(); pp, _ := p.Ppid(); ct, _ := p.CreateTime(); ccp := 0.0
			if times, err := p.Times(); err == nil {
				totalCPU := times.Total()
				if prev, ok := procCPUSamples[p.Pid]; ok && prev.createTime == ct {
					dt := now.Sub(prev.sampleTime).Seconds()
					if dt > 0 { ccp = ((totalCPU - prev.totalCPU) / dt) * 100 / cpuScale; if ccp < 0 || math.IsNaN(ccp) || math.IsInf(ccp, 0) { ccp = 0 } }
				}
				if ct > 0 { procCPUSamples[p.Pid] = procCPUSample{createTime: ct, totalCPU: totalCPU, sampleTime: now} }
			}
			mp, _ := p.MemoryPercent(); u, _ := p.Username(); cmdl, _ := p.Cmdline(); gmem, gid, gutil := uint32(0), uint32(0), uint32(0)
			if info, ok := gm[p.Pid]; ok { gmem, gid, gutil = info.mem, info.gpu, info.util }
			minF, majF := uint64(0), uint64(0); if faults, err := p.PageFaults(); err == nil && faults != nil { minF = faults.MinorFaults; majF = faults.MajorFaults }
			currentPIDs[p.Pid] = struct{}{}; stats.Processes = append(stats.Processes, &pb.Process{Pid: p.Pid, Ppid: pp, Name: n, Cpu: ccp, Mem: mp, User: u, GpuMem: gmem, GpuId: gid, GpuUtil: gutil, Cmdline: cmdl, CreateTime: ct, MinorFaults: minF, MajorFaults: majF})
		}
		for pid := range procCPUSamples { if _, ok := currentPIDs[pid]; !ok { delete(procCPUSamples, pid) } }
