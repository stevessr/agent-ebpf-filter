...
> Source: https://docs.ebpf.io/linux/helper-function/bpf_probe_read_kernel_str/
> Cached: 2026-04-28

[Skip to content](#helper-function-bpf_probe_read_kernel_str)
...
[v5.5](https://github.com/torvalds/linux/commit/6ae08ae3dea2cfa03dd3665a3c8475c2d429ef47)
...
```
 SEC("tracepoint/syscalls/sys_exit_openat") int  trace_open(struct  trace_event_raw_sys_exit  * ctx)  { char  comm[256];  struct  task_struct  * task  =  (struct  task_struct  *) bpf_get_current_task();  // Try to read the process name and check for errors  int  ret  =  bpf_probe_read_kernel_str(comm,  sizeof(comm),  task -> comm);  if  (ret  <  0)  { bpf_printk("Failed to read process name, error: %d \n ",  ret);  return  0;  }  bpf_printk("Process name: %s \n ",  comm);  return  0;}
```
