...
import {
  AppstoreOutlined, CodeOutlined, PlusOutlined, ReloadOutlined,
  DeleteOutlined, ThunderboltOutlined, SaveOutlined, PoweroffOutlined,
} from '@ant-design/icons-vue';
import { Modal, message } from 'ant-design-vue';
import { usePlugins, type PluginManifest, type PluginAttachKind, type BPFTemplate } from '../composables/usePlugins';

const {
  plugins, templates, compileLog, lastCompile, loading,
...
const resetBuilder = () => {
  builder.value = {
    ...
    attachKind: 'tracepoint', attachTarget: 'syscalls/sys_enter_execve',
    programName: 'trace_execve', enabled: false, source: '',
  };
  compileLog.value = '';
...
const sortedPlugins = computed(() => [...plugins.value].sort((a, b) => a.id.localeCompare(b.id)));
...
watch(() => builder.value.programName, (val) => {
...
<template>
  <div style="padding: 24px; background: #f0f2f5; min-height: 100%">
    <a-tabs v-model:activeKey="activeTab" type="card" size="large">
      <!-- ─── 插件列表 ─── -->
      <a-tab-pane key="list">
        <template #tab><span><AppstoreOutlined /> 插件管理</span></template>
        ...
        <a-card>
          ...
          <template #extra>
            <a-space>
              ...
              <a-button type="primary" @click="activeTab = 'builder'; resetBuilder()">
          ...
          <a-empty v-if="!sortedPlugins.length" description="暂无插件，前往“在线 eBPF 制作”创建一个" />
            ...
            <template #renderItem="{ item }">
              <a-list-item :key="item.id">
                <template #actions>
                  ...
                  />
                  <a-button size="small" @click="handleLoadIntoBuilder(item.id)">
                ...
                <a-list-item-meta>
                  <template #title>
                    <a-space>
                      ...
                      <a-tag :color="statusTag(item).color">{{ statusTag(item).text }}</a-tag>
                      <a-tag v-if="item.attachKind">{{ item.attachKind }}: {{ item.attachTarget }}</a-tag>
                    </a-space>
                  </template>
                  <template #description>
                    ...
                    <div style="color:#999; font-size:12px; margin-top: 4px">
                      ...
                      <span v-if="item.programName"> · 程序: <code>{{ item.programName }}</code></span>
      ...
      <a-tab-pane key="builder">
        <template #tab><span><CodeOutlined /> 在线 eBPF 制作</span></template>
              ...
              <a-list :data-source="templates" size="small">
                <template #renderItem="{ item }">
                  <a-list-item :key="item.id">
                    <a-list-item-meta>
                      <template #title>
                        <a @click="applyTemplate(item)" style="font-weight:600">{{ item.name }}</a>
                      </template>
                      <template #description>
                        <div style="font-size:12px">{{ item.description }}</div>
                        <a-tag style="margin-top:4px">{{ item.attachKind }}: {{ item.attachTarget }}</a-tag>
                      </template>
                    </a-list-item-meta>
                  </a-list-item>
                </template>
            ...
            <a-card title="元数据" size="small" style="margin-top: 12px">
              <a-form layout="vertical" :model="builder">
                <a-form-item label="插件 ID (a-z0-9-)">
                  <a-input v-model:value="builder.id" placeholder="例如 my-trace-exec" />
                </a-form-item>
                ...
                <a-form-item label="附挂类型">
                  <a-select v-model:value="builder.attachKind" :options="attachKindOptions" />
                </a-form-item>
                <a-form-item label="附挂目标">
                  <a-input v-model:value="builder.attachTarget" :placeholder="builder.attachKind === 'tracepoint' ? 'syscalls/sys_enter_xxx' : 'do_unlinkat'" />
                </a-form-item>
                <a-form-item label="程序入口 (SEC 名)">
                  <a-input v-model:value="builder.programName" placeholder="例如 trace_execve" />
                </a-form-item>
                <a-form-item>
                  <a-checkbox v-model:checked="builder.enabled">保存后立即启用</a-checkbox>
                </a-form-item>
              </a-form>
            </a-card>
          </a-col>
            ...
            <a-card title="源码 (C / libbpf)" size="small">
              <template #extra>
                <a-space>
                  ...
                  <a-button v-if="lastCompile" danger @click="unloadBpf(builder.id)">卸载</a-button>
                </a-space>
              </template>
              <a-textarea
                ...
                style="font-family: 'JetBrains Mono', Menlo, Consolas, monospace; font-size: 12px"
              />
              <a-alert
                v-if="lastCompile"
                ...
                show-icon
              />
              <a-card v-if="compileLog" size="small" title="编译输出" style="margin-top: 12px; background:#1f1f1f">
                <pre style="margin:0; color:#fafafa; font-size:12px; max-height: 240px; overflow:auto">{{ compileLog }}</pre>
              </a-card>
            </a-card>

            <a-card size="small" style="margin-top: 12px" title="编写提示">
              <ul style="margin:0; padding-left: 20px; color:#666">
                <li>仅允许 <code>#include &quot;vmlinux.h&quot;</code> 与 <code>&lt;bpf/...&gt;</code> 形式的头文件。</li>
                <li>必须声明至少一个 <code>SEC(&quot;...&quot;)</code> 程序。</li>
                <li>需要在主机上安装 <code>clang &ge; 14</code>，并允许后端访问 <code>/sys/fs/bpf</code>。</li>
                <li>Tracepoint 目标格式：<code>category/name</code>，例如 <code>syscalls/sys_enter_openat</code>。</li>
                <li>Kprobe / Kretprobe 目标为函数名，例如 <code>do_unlinkat</code>。</li>
              </ul>
            </a-card>
          </a-col>
        </a-row>
