...
const toggleHook = async (hook: HookDef) => {
  try {
    ...
    message.success(
      `${hook.installed ? "Uninstalled" : "Installed"} hook for ${hook.name}`,
    );
    await fetchHooks();
  } catch {
...
<template>
  <div style="background: #f0f2f5; padding: 24px; min-height: 100%">
    <div style="max-width: 1000px; margin: 0 auto">
      <a-card
        :bordered="false"
        style="border-radius: 8px; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05)"
      >
        <template #title>
          <div style="display: flex; align-items: center; gap: 8px">
            <LinkOutlined style="color: #1890ff; font-size: 18px" />
            <span style="font-size: 16px; font-weight: bold"
              >AI CLI Interception Hooks</span
            >
          </div>
        </template>
        <template #extra>
          <a-button @click="fetchHooks" :loading="loading" size="small"
            >Refresh</a-button
          >
        </template>
        ...
        >
          <template #description>
            <div>
              ...
              directory
              <code
                >~/.gemini/antigravity-cli/plugins/agent-ebpf-hook-active</code
          ...
          <template #renderItem="{ item }">
            <a-list-item>
              <HookCard
                ...
                :loading="loading"
                @update:use-wrapper="
                  (value) => (useWrapperOverride[item.id] = value)
                "
                @toggle="toggleHook"
                @edit="openEditModal"
              />
