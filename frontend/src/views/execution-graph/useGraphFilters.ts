...
export const timePresetOptions: ExecutionGraphFilterState["timePreset"][] = [
  "all",
  "15m",
  "1h",
  "6h",
...
export const filtersFromRoute = (
  ...
  const timePreset = String(
    singleQuery(query.timePreset || query.time_preset || defaults.timePreset),
  ).trim() as ExecutionGraphFilterState["timePreset"];
  const since = String(singleQuery(query.since)).trim();
  const until = String(singleQuery(query.until)).trim();
  const processTreeRaw = String(singleQuery(query.process_tree))
    .trim()
    .toLowerCase();
  const parsedTimePreset = timePresetOptions.includes(timePreset)
    ? timePreset
    : defaults.timePreset;
  return {
    ...defaults,
    ...
    processTree:
      processTreeRaw === ""
        ? defaults.processTree
        : ["1", "true", "yes", "on"].includes(processTreeRaw),
    ...
    riskMin:
      Number.isFinite(parsedRisk) && parsedRisk > 0
        ...
        : defaults.riskMin,
    timePreset:
      (since || until) && parsedTimePreset !== "custom"
        ? "custom"
        : parsedTimePreset,
    since,
    until,
