...
import {
  useCodingLauncher,
  CODING_PRESET_OPTIONS,
} from "../../composables/executor/useCodingLauncher";
import {
  useScriptLauncher,
  SCRIPT_LANGUAGE_OPTIONS,
} from "../../composables/executor/useScriptLauncher";
import {
  dirname,
  resolvePythonInterpreter,
} from "../../composables/executor/useLauncherUtils";
...
const scriptTabKey = ref<ScriptTabKey>(
  route.params.tab === "launch-env"
    ? "launch-env"
    : normalizeScriptTab(route.params.subtab),
);
  ...
  if (val !== route.params.tab) {
    router.replace({
      ...
      params: {
        tab: val,
        subtab:
          val === "scripts" && scriptTabKey.value !== "runner"
            ? scriptTabKey.value
            : undefined,
      },
    });
...
const isTmuxSession = (session: ShellSessionInfo) => {
  ...
  return (
    (session.shell || "").trim().toLowerCase() === "tmux" ||
    (session.command || "").trim().toLowerCase() === "tmux"
  );
};
...
const getPathPickerInitialPath = computed(() => {
  switch (pathPickerTarget.value) {
    ...
    case "python-venv":
      return (
        pythonVenv.value.trim() ||
        scriptWorkDir.value.trim() ||
        dirname(scriptPath.value) ||
        "/"
      );
    default:
      return "/";
...
const getPathPickerMode = computed(() =>
  pathPickerTarget.value === "script-path" ? "file" : "directory",
);
  ...
  <div style="padding: 24px; background: #f0f2f5; min-height: 100%">
    ...
    >
            ...
            <a-card title="Launch coding CLI in tmux" :bordered="false">
              ...
              <a-form layout="vertical">
                <a-form-item label="CLI preset">
                  <a-select
                    v-model:value="codingPreset"
                    :options="CODING_PRESET_OPTIONS"
                  />
                </a-form-item>
                <a-form-item
                  v-if="codingPreset === 'custom'"
                ...
                <a-form-item label="Launch mode">
                  <a-space>
                    ...
                    <span>{{
                      codingUseTmux ? "tmux wrapper" : "direct command"
                    }}</span>
                  </a-space>
                </a-form-item>
                <a-alert
      ...
      <a-tab-pane key="scripts" tab="Script Runner">
        <a-tabs
          v-model:activeKey="scriptTabKey"
          size="small"
          :destroyInactiveTabPane="false"
        >
              ...
              <a-col :xs="24" :xl="10">
                <a-card title="Launch script" :bordered="false">
                  ...
                  <a-form layout="vertical">
                    ...
                    <a-form-item
                      v-if="scriptLanguage === 'python'"
                      label="Python venv directory"
                    >
                      <a-input-search
                  ...
                  <a-space direction="vertical" :size="12" style="width: 100%">
                    <a-descriptions bordered size="small" :column="1">
                      <a-descriptions-item label="Default environment"
                        ><span
                          >System + launch env overrides</span
                        ></a-descriptions-item
                      >
                      <a-descriptions-item label="Python interpreter"
                        ><span>{{
                          resolvePythonInterpreter(pythonVenv)
                      ...
                      >
                      <a-descriptions-item label="Launch env"
                        ><span>{{
                          launchEnvPreview
                        }}</span></a-descriptions-item
                      >
