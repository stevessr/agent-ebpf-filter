...
export const isLocalClusterTarget = (target?: string | null) =>
  normalizeClusterTarget(target) === LOCAL_CLUSTER_TARGET;
...
export const getStoredClusterTarget = () => {
  ...
  return normalizeClusterTarget(
    window.localStorage.getItem(CLUSTER_TARGET_STORAGE_KEY),
  );
};
...
export const setStoredClusterTarget = (target: string) => {
  ...
  window.dispatchEvent(
    new CustomEvent(CLUSTER_TARGET_CHANGED_EVENT, {
...
export const fetchProto = async <T>(
  url: string,
