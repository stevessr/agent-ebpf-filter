import { decodeStdioMessage, formatBytes, formatDuration, type ProcessedAgentSightEvent } from './agentsight';

export type AgentSightFlameMetric = 'count' | 'bytes' | 'duration' | 'risk';
export type AgentSightFlameDimensionPreset = 'execution' | 'process' | 'conversation';
...
function segment(level: string, value: unknown, fallback: string): AgentSightFlameSegment {
...
function readEventField(event: ProcessedAgentSightEvent, keys: string[]) {
  return readAny(event.data, keys)
...
function processLabel(event: ProcessedAgentSightEvent) {
  const comm = event.comm || String(readEventField(event, ['comm', 'Comm']) || 'process');
...
function executionScopeLabel(event: ProcessedAgentSightEvent) {
  const agentRunId = firstNonEmpty(readEventField(event, ['agent_run_id', 'agentRunId', 'AgentRunId']));
  if (agentRunId) return `agent ${agentRunId}`;
  const conversationId = firstNonEmpty(readEventField(event, ['conversation_id', 'conversationId', 'ConversationId']));
  if (conversationId) return `conversation ${conversationId}`;
  const traceId = event.traceId || firstNonEmpty(readEventField(event, ['trace_id', 'traceId', 'TraceId']));
  if (traceId) return `trace ${traceId}`;
  return 'unscoped';
}

function conversationLabel(event: ProcessedAgentSightEvent) {
  return firstNonEmpty(readEventField(event, ['conversation_id', 'conversationId', 'ConversationId']), event.traceId) || 'conversation unknown';
}

function turnLabel(event: ProcessedAgentSightEvent) {
  return firstNonEmpty(readEventField(event, ['turn_id', 'turnId', 'TurnId']), readEventField(event, ['task_id', 'taskId', 'TaskId'])) || 'turn unknown';
}
...
function toolLabel(event: ProcessedAgentSightEvent) {
  ...
  if (event.source === 'stdio') {
    ...
    return compactLabel(decoded.toolName || decoded.method || decoded.title || decoded.fdRole || 'stdio', 72);
  }
...
function networkEndpoint(event: ProcessedAgentSightEvent) {
  const dstIp = firstNonEmpty(readEventField(event, ['dst_ip', 'dstIp', 'DstIp']));
  const dstPort = firstNonEmpty(readEventField(event, ['dst_port', 'dstPort', 'DstPort']));
  if (dstIp && dstPort) return `${dstIp}:${dstPort}`;
  return firstNonEmpty(
    readEventField(event, ['domain', 'dns_name', 'dnsName', 'sni', 'http_host', 'httpHost', 'host']),
...
export function extractAgentSightFlameTarget(event: ProcessedAgentSightEvent) {
  ...
  if (source === 'stdio') {
    const decoded = decodeStdioMessage(event.data);
    return compactLabel(decoded.toolName || decoded.method || decoded.preview || decoded.summary || decoded.title || event.title, 96);
  }
  if (source === 'file') {
    return compactLabel(firstNonEmpty(
      readEventField(event, ['path', 'Path', 'filepath', 'filePath', 'file_path']),
      readEventField(event, ['extra_path', 'extraPath', 'ExtraPath']),
      event.title,
    ), 120);
  }
  if (source === 'network' || source === 'ssl' || source === 'http_parser' || source === 'sse_processor') {
    const host = firstNonEmpty(readEventField(event, ['host', 'domain', 'dns_name', 'dnsName', 'sni', 'http_host', 'httpHost']));
    ...
    return compactLabel(firstNonEmpty(host && path ? `${method ? `${method} ` : ''}${host}${path.startsWith('/') ? path : `/${path}`}` : '', endpoint, path, event.title), 120);
  }
  if (source === 'policy') {
    return compactLabel(firstNonEmpty(
...
export function extractAgentSightFlameMetrics(event: ProcessedAgentSightEvent): AgentSightFlameMetrics {
  ...
  const risk = explicitRisk > 0
    ? explicitRisk <= 1 ? explicitRisk * 100 : explicitRisk
    : decision.includes('alert') ? 100
      : decision.includes('block') || decision.includes('deny') ? 80
        : decision.includes('warn') ? 50
          : 0;
  return {
    count: 1,
    bytes,
...
export function buildAgentSightFlamePath(event: ProcessedAgentSightEvent, preset: AgentSightFlameDimensionPreset): AgentSightFlameSegment[] {
  if (preset === 'process') {
    return [
...
function addMetrics(target: AgentSightFlameMetrics, delta: AgentSightFlameMetrics) {
...
function metricValue(metrics: AgentSightFlameMetrics, metric: AgentSightFlameMetric) {
  return metrics[metric] || 0;
}

function updateNode(node: MutableAgentSightFlameNode, event: ProcessedAgentSightEvent, metrics: AgentSightFlameMetrics) {
  addMetrics(node.metrics, metrics);
  node.eventCount += 1;
  if (!node.latestTimestamp || event.timestamp > node.latestTimestamp) node.latestTimestamp = event.timestamp;
  if (!node.representativeEvent || metricValue(metrics, 'risk') > metricValue(extractAgentSightFlameMetrics(node.representativeEvent), 'risk') || event.timestamp > (node.representativeEvent.timestamp || 0)) {
    node.representativeEvent = event;
  }
  if (node.sampleEvents.length < SAMPLE_EVENT_LIMIT) node.sampleEvents.push(event);
  const currentSource = node.sourceWeights.get(event.source) || { count: 0, color: event.sourceColor || '#64748b' };
...
function finalizeNode(node: MutableAgentSightFlameNode): AgentSightFlameNode {
  let dominant = { source: node.dominantSource, count: 0, color: node.dominantColor };
  node.sourceWeights.forEach((value, source) => {
    if (value.count > dominant.count) dominant = { source, count: value.count, color: value.color };
  });
...
export function buildAgentSightFlameTree(events: ProcessedAgentSightEvent[], preset: AgentSightFlameDimensionPreset): AgentSightFlameNode {
  ...
  events.forEach(event => {
    ...
    buildAgentSightFlamePath(event, preset).forEach((pathSegment, index) => {
      let child = current.children.find(item => item.key === pathSegment.key) as MutableAgentSightFlameNode | undefined;
      if (!child) {
...
function cloneSyntheticOther(children: AgentSightFlameNode[], parent: AgentSightFlameNode, metric: AgentSightFlameMetric): AgentSightFlameNode {
  const other = children.reduce<AgentSightFlameNode>((node, child) => {
    addMetrics(node.metrics, child.metrics);
    node.eventCount += child.eventCount;
    if (!node.latestTimestamp || (child.latestTimestamp || 0) > node.latestTimestamp) node.latestTimestamp = child.latestTimestamp;
    if (!node.representativeEvent && child.representativeEvent) node.representativeEvent = child.representativeEvent;
...
function visibleChildren(node: AgentSightFlameNode, metric: AgentSightFlameMetric, parentWidth: number, minSegmentPx: number, maxChildren: number) {
  const total = Math.max(metricValue(node.metrics, metric), 0);
  if (total <= 0) return [];
  const sorted = node.children.slice().sort((a, b) => metricValue(b.metrics, metric) - metricValue(a.metrics, metric));
  ...
  if (hidden.length > 0) visible.push(cloneSyntheticOther(hidden, node, metric));
  return visible;
}

export function layoutAgentSightFlamegraph(root: AgentSightFlameNode, metric: AgentSightFlameMetric, options: AgentSightFlameLayoutOptions = {}): AgentSightFlameRect[] {
  ...
  const visit = (node: AgentSightFlameNode, x: number, y: number, nodeWidth: number, parentValue: number) => {
    const nodeValue = Math.max(metricValue(node.metrics, metric), 0);
    let offset = x;
    visibleChildren(node, metric, nodeWidth, minSegmentPx, maxChildren).forEach(child => {
...
export function findAgentSightFlameNode(root: AgentSightFlameNode, id: string): AgentSightFlameNode | null {
...
export function agentSightFlameBreadcrumbs(root: AgentSightFlameNode, id: string): AgentSightFlameNode[] {
...
export function agentSightFlameMetricValue(node: AgentSightFlameNode, metric: AgentSightFlameMetric) {
  return metricValue(node.metrics, metric);
}

export function formatAgentSightFlameMetric(value: number, metric: AgentSightFlameMetric) {
  if (metric === 'bytes') return formatBytes(value);
  if (metric === 'duration') return formatDuration(value);
  if (metric === 'risk') return value >= 10 ? value.toFixed(0) : value.toFixed(1);
