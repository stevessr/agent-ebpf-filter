...
export const hookCatalog: Record<string, HookCliDoc> = {
  ...
  augment: {
    ...
    sources: [
      { label: 'Augment hooks reference', url: 'https://docs.augmentcode.com/cli/hooks' },
      { label: 'Augment hooks examples', url: 'https://docs.augmentcode.com/cli/hooks-examples' },
    ],
    commonFields: [
      { name: 'hook_event_name', type: 'string', description: 'PreToolUse, PostToolUse, Stop, SessionStart, SessionEnd' },
      { name: 'conversation_id', type: 'string', description: 'Unique identifier for the current conversation' },
      { name: 'workspace_roots', type: 'string[]', description: 'Workspace root directories (usually one path)' },
    ],
    notes: [
    ...
    events: [
      { name: 'PreToolUse', description: 'Runs before a tool executes; can block via exit code 2 or permissionDecision: "deny"', matcher: 'tool name (regex)', fields: [{ name: 'tool_name', type: 'string', description: 'Tool name (e.g. launch-process, str-replace-editor, save-file, view, web-fetch)' }, { name: 'tool_input', type: 'object', description: 'Tool arguments; schema depends on the tool' }] },
      { name: 'PostToolUse', description: 'Runs after a tool completes; cannot block but can inject context', matcher: 'tool name (regex)', fields: [{ name: 'tool_name', type: 'string', description: 'Executed tool name' }, { name: 'tool_input', type: 'object', description: 'Arguments sent to the tool' }, { name: 'tool_output', type: 'string?', description: 'Tool output if successful' }, { name: 'tool_error', type: 'string?', description: 'Error message if the tool failed' }, { name: 'file_changes', type: 'object[]?', description: 'For save-file/str-replace-editor/remove-files: path, changeType, content, oldContent' }] },
      { name: 'Stop', description: 'Runs when the agent stops responding; can block stop with decision: "block"', fields: [{ name: 'agent_stop_cause', type: 'string', description: '"end_turn" or "interrupted"' }] },
      { name: 'SessionStart', description: 'Runs when a new Auggie session begins; stdout is injected as agent context', notes: ['Session events do not use matchers.'] },
      { name: 'SessionEnd', description: 'Runs when an Auggie session ends', notes: ['Session events do not use matchers.'] },
    ],
  },
  antigravity: {
    id: 'antigravity',
    ...
    sources: [
      { label: 'Google Antigravity hooks documentation', url: 'https://www.antigravity.google/docs/hooks' },
      { label: 'Antigravity CLI plugins documentation', url: 'https://www.antigravity.google/docs/cli-features' },
      { label: 'Antigravity CLI settings documentation', url: 'https://www.antigravity.google/docs/cli-using' },
    ],
    commonFields: [
      { name: 'conversationId', type: 'string', description: 'Active Antigravity agent conversation UUID' },
      { name: 'workspacePaths', type: 'string[]', description: 'Mounted workspace directories for the conversation' },
      { name: 'transcriptPath', type: 'string', description: 'Path to transcript.jsonl for the conversation' },
      { name: 'artifactDirectoryPath', type: 'string', description: 'Directory containing artifacts and screenshots' },
    ],
    notes: [
    ...
    events: [
      { name: 'PreToolUse', description: 'Before a tool executes; can allow, deny, ask, or force_ask', matcher: 'tool name regex', fields: [{ name: 'toolCall.name', type: 'string', description: 'Tool name such as run_command' }, { name: 'toolCall.args', type: 'object', description: 'Tool arguments; run_command uses CommandLine, Cwd, WaitMsBeforeAsync, and related fields' }, { name: 'stepIdx', type: 'number', description: '0-based step index in the trajectory' }], notes: ['The generated relay returns decision=allow so agent-ebpf observes without changing Antigravity permissions.'] },
      { name: 'PostToolUse', description: 'After a tool completes; returns an empty JSON object', matcher: 'tool name regex', fields: [{ name: 'stepIdx', type: 'number', description: '0-based completed step index' }, { name: 'error', type: 'string', description: 'Runtime error string if the tool failed' }] },
      { name: 'PreInvocation', description: 'Before the model is called; can inject trajectory steps', fields: [{ name: 'invocationNum', type: 'number', description: 'Current model invocation sequence number' }, { name: 'initialNumSteps', type: 'number', description: 'Trajectory length before invocation' }] },
      { name: 'PostInvocation', description: 'After tool calls finish; can inject steps or terminate / force_continue', fields: [{ name: 'invocationNum', type: 'number', description: 'Current model invocation sequence number' }, { name: 'initialNumSteps', type: 'number', description: 'Trajectory length before invocation' }] },
      { name: 'Stop', description: 'When the execution loop terminates', fields: [{ name: 'executionNum', type: 'number', description: 'Execution attempt sequence number' }, { name: 'terminationReason', type: 'string', description: 'Reason such as model_stop, max_steps_exceeded, or error' }, { name: 'fullyIdle', type: 'boolean', description: 'Whether all background commands or tasks are finished' }] },
    ],
  },
  copilot: {
    id: 'copilot',
