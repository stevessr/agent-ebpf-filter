  ...
  >
    <a-descriptions bordered :column="1" size="small" v-if="event">
      ...
      <a-descriptions-item label="Direction">
        <a-tag :color="directionColor(event.netDirection)">{{
          formatDirection(event.netDirection)
        }}</a-tag>
      </a-descriptions-item>
      <a-descriptions-item label="Event Type">
        <a-tag :color="typeColor(event.eventType, event.type)">{{
      ...
      <a-descriptions-item label="PID"
        ><code>{{ formatDetailValue(event.pid) }}</code></a-descriptions-item
      >
      <a-descriptions-item label="Parent PID (PPID)"
        ><code>{{ formatDetailValue(event.ppid) }}</code></a-descriptions-item
      >
      <a-descriptions-item label="User ID (UID)"
        ><code>{{ formatDetailValue(event.uid) }}</code></a-descriptions-item
      >
      <a-descriptions-item label="Endpoint"
        ><code style="word-break: break-all">{{
          formatDetailValue(event.netEndpoint)
        }}</code></a-descriptions-item
      >
      <a-descriptions-item label="Family">
        <a-tag :color="familyColor(event.netFamily)">{{
          event.netFamily || "unknown"
        }}</a-tag>
      </a-descriptions-item>
      <a-descriptions-item label="Bytes">{{
        formatBytes(event.netBytes)
      }}</a-descriptions-item>
      <a-descriptions-item label="Summary"
        ><code style="word-break: break-all">{{
          formatDetailValue(event.path)
        }}</code></a-descriptions-item
      >
    </a-descriptions>
  </a-modal>
