...
const formatRate = (bytesPerSecond?: number) =>
  `${formatBytes(bytesPerSecond || 0)}/s`;
    ...
    <a-card title="网络流分析" size="small" :loading="loading">
      <template #extra>
        <a-space>
          ...
          <a-tag v-if="tcpConns.length" color="green"
            >{{ tcpConns.filter((c) => c.state === "ESTABLISHED").length }} 活跃
            TCP</a-tag
          >
        </a-space>
      </template>
          ...
          <div class="flow-filter-bar">
            <a-space wrap>
              ...
              <a-switch
                ...
                @change="refreshNetworkState"
              />
              <a-button size="small" @click="refreshNetworkState"
                >刷新</a-button
              >
            </a-space>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'app'">
                <a-space :size="4" wrap>
                  ...
                  >
                  <a-tag v-if="record.dstService" color="blue" size="small">{{
                    record.dstService
                  }}</a-tag>
                </a-space>
              </template>
              <template v-else-if="column.key === 'domain'">
                <div style="display: flex; flex-direction: column; gap: 4px">
                  ...
                  <a-space :size="4" wrap>
                    ...
                    <a-tag
                      v-if="
                        record.dnsName && record.dnsName !== record.dstDomain
                      "
                      color="purple"
                      size="small"
              ...
              <template v-else-if="column.key === 'rate'">
                <span style="font-family: monospace; font-size: 12px"
                  >↑{{ formatRate(record.currentBpsOut) }}</span
                >
              </template>
              <template v-else-if="column.key === 'stale'">
                <a-tag :color="staleColor(record.staleLevel)" size="small">{{
                  record.staleLevel || "active"
                }}</a-tag>
              </template>
        ...
        <a-tab-pane key="tcp" tab="TCP 状态">
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template
                v-else-if="column.key === 'src' || column.key === 'dst'"
              >
                <span style="font-family: monospace; font-size: 12px">{{
                  record[column.key as keyof TCPConnection]
                }}</span>
              </template>
            </template>
          </a-table>
      ...
      <div v-if="error" style="color: #ff4d4f; margin-top: 8px">
        {{ error }}
      </div>
    </a-card>
  </div>
