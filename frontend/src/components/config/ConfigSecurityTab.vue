...
import ConfigCgroupSandboxCard from "./security/ConfigCgroupSandboxCard.vue";
import ConfigLsmEnforcerCard from "./security/ConfigLsmEnforcerCard.vue";
import {
...
<template>
  <a-row :gutter="[24, 24]">
      ...
      <a-card title="Wrapper Security Policies" size="small">
        ...
        >
          ...
          >
            ...
            <a-button size="small" type="link" @click="addAllQuickRulePresets"
              >一键添加全部</a-button
            >
          </div>
          <a-space wrap>
            ...
            >
              <a-button
                ...
                :style="
                  preset.action === 'ALERT'
                    ? 'border-color: #faad14; color: #d48806;'
                    : ''
                "
                @click="addQuickRulePreset(preset)"
              >
                <component
                  :is="preset.action === 'BLOCK' ? StopOutlined : AlertOutlined"
                />
        ...
        >
          ...
          <a-row :gutter="[12, 12]">
            <a-col
              v-for="source in externalRuleSources"
              :key="source.id"
              :xs="24"
              :sm="8"
            >
              <a-card size="small" :hoverable="true">
                ...
                <div style="display: flex; gap: 6px">
                  <a-tag
                    :color="source.category === 'owasp' ? 'orange' : 'blue'"
          ...
          <div v-if="fetchedExternalRules.length > 0" style="margin-top: 12px">
            ...
            >
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'comm'"
                  ><code>{{ record.comm }}</code></template
                >
            ...
            <div style="margin-top: 8px; text-align: right">
              ...
              >
                <ImportOutlined /> 一键导入
                {{ fetchedExternalRules.length }} 条规则
              </a-button>
            </div>
          </div>
        </div>
        ...
        <a-table
          :dataSource="
            Object.values(wrapperRules).sort(
        ...
        >
          ...
          >
            <template #default="{ text }"
              ><a-tag color="blue">{{ text || 0 }}</a-tag></template
            >
          </a-table-column>
          <a-table-column
    ...
    <ConfigCgroupSandboxCard :security="props.security" />

    <ConfigLsmEnforcerCard :security="props.security" />
            ...
            >
              ...
              >
                ...
                <span style="margin-left: auto; font-size: 11px; opacity: 0.85"
                  >{{
                    group.syscalls.filter(
                      (s) => !disabledEventTypes.has(s.type),
                    ).length
                  }}/{{ group.syscalls.length }}</span
                >
              </div>
              <div style="padding: 0">
                <div
                  v-for="s in group.syscalls"
                  :key="s.type"
                  style="
                    display: flex;
                    align-items: center;
                  ...
                  "
                  :style="
                    disabledEventTypes.has(s.type) ? 'opacity: 0.45;' : ''
                  "
                >
                  ...
                  <a-switch
                    ...
                    size="small"
                    @change="
                      toggleEventType(s.type, disabledEventTypes.has(s.type))
                    "
                  >
