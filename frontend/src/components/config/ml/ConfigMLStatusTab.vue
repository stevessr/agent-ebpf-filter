      ...
      <a-row :gutter="[12, 12]">
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              title="ML Engine"
              :value="mlEnabled ? 'Active' : 'Inactive'"
              :value-style="{
                color: mlEnabled ? '#3f8600' : '#cf1322',
                fontSize: '18px',
              }"
            />
          </a-card>
        ...
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              title="Model Loaded"
              :value="mlStatus.model_loaded ? 'Yes' : 'No'"
              :value-style="{
                color: mlStatus.model_loaded ? '#3f8600' : '#d48806',
                fontSize: '18px',
              }"
            />
          </a-card>
            ...
            <a-statistic
              ...
              :value="
                mlStatus.train_accuracy
                  ? (mlStatus.train_accuracy * 100).toFixed(1)
                  : '—'
              "
              :suffix="mlStatus.train_accuracy ? '%' : ''"
            />
          </a-card>
        </a-col>
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              ...
              :value="
                mlStatus.validation_accuracy
                  ? (mlStatus.validation_accuracy * 100).toFixed(1)
                  : '—'
              "
              :suffix="mlStatus.validation_accuracy ? '%' : ''"
            />
          </a-card>
        </a-col>
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              title="Train Samples"
              :value="mlStatus.train_samples || 0"
            />
          </a-card>
        </a-col>
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              ...
              :value="mlStatus.validation_samples || 0"
            />
          </a-card>
        </a-col>
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              ...
              :value="mlStatus.num_labeled_samples || 0"
            />
          </a-card>
        </a-col>
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              ...
              :value="((mlStatus.validation_split_ratio || 0) * 100).toFixed(0)"
              suffix="%"
            />
          </a-card>
        </a-col>
        <a-col :xs="12" :sm="8" :md="6">
          ...
          >
            <a-statistic
              title="Last Trained"
              :value="mlStatus.last_trained || 'Never'"
  ...
  <a-col :xs="24">
    <a-card size="small">
      <template #title>
        ...
        <a-tag
          v-else-if="trainingLogs.length > 0"
          color="green"
          style="margin-left: 8px"
        >
      ...
      >
        <template v-if="trainingLogs.length > 0">
          ...
          >
            ...
            >
            <span
              v-else-if="line.message.startsWith('═══')"
              style="color: #569cd6; font-weight: bold"
              >{{ " " + line.message }}</span
            >
            <span
              v-else-if="
                line.message.includes('完成') ||
                line.message.includes('complete') ||
                line.message.includes('accuracy')
              "
              style="color: #89d185"
              >{{ " " + line.message }}</span
            >
    ...
    <a-card title="LLM Post-Training Review" size="small">
      ...
      <a-descriptions :column="3" size="small" bordered>
        <a-descriptions-item label="Source">{{
          mlStatus.llm_review?.source || "—"
        }}</a-descriptions-item>
        <a-descriptions-item label="Model">{{
          mlStatus.llm_review?.model || llmScoringConfig.model || "—"
        }}</a-descriptions-item>
        <a-descriptions-item label="Scored Samples">{{
          mlStatus.llm_review?.scoredSamples ?? 0
        }}</a-descriptions-item>
        <a-descriptions-item label="Average Risk">{{
          mlStatus.llm_review
            ? mlStatus.llm_review.averageRiskScore.toFixed(1)
            : "—"
        }}</a-descriptions-item>
        <a-descriptions-item label="Agreement">{{
          mlStatus.llm_review
            ? (mlStatus.llm_review.agreement * 100).toFixed(0) + "%"
            : "—"
        }}</a-descriptions-item>
        <a-descriptions-item label="Validation Split">{{
          mlStatus.llm_review?.validationSplitRatio !== undefined
            ? (mlStatus.llm_review.validationSplitRatio * 100).toFixed(0) + "%"
            : "—"
        }}</a-descriptions-item>
        <a-descriptions-item label="Reviewed At" :span="3">{{
          mlStatus.llm_review?.reviewedAt
            ? new Date(mlStatus.llm_review.reviewedAt).toLocaleString()
            : "—"
        }}</a-descriptions-item>
      </a-descriptions>
    </a-card>
