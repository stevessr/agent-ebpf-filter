...
const sampleMatchRowKey = (record: any) =>
  `${record.commandLine || ""}:${record.label || ""}:${record.userLabel || ""}:${record.index ?? ""}`;
...
<template>
    ...
    <a-card title="Training Controls" size="small">
      <a-space direction="vertical" style="width: 100%">
        ...
        >
          ...
          >
            <span
              ><ReloadOutlined
                spin
                style="margin-right: 4px; color: #52c41a"
              /><b>训练中</b>
              {{ Math.round((mlStatus.training_progress || 0) * 100) }}%</span
            >
            <span
              v-if="cudaAvailable && cudaMemTotalMB > 0"
              style="color: #4a4a4a"
        ...
        <a-alert
          ...
          :description="
            trainingReadinessBlockText ||
            '请先导入/标注更多样本并刷新 ML 状态。'
          "
        />
        <div style="display: flex; gap: 8px">
          <a-button
            ...
            :disabled="
              mlStatus.training_in_progress || trainingReadinessBlocked
            "
            style="flex: 1"
          >
        ...
        >
          ...
          >
            <div style="font-weight: 600">
              C running time
              <a-tag
                :color="mlCRuntime.cSupported ? 'green' : 'orange'"
            ...
            <a-tag color="geekblue"
              >active:
              {{ runtimeBackendLabel(mlCRuntime.activeBackend) }}</a-tag
            >
          </div>
          <a-space wrap size="small" style="margin-bottom: 8px">
            <a-tooltip
              v-for="backend in mlCRuntime.backends || []"
              :key="backend.id"
              :title="backend.detail || backend.label"
            >
          ...
          <a-descriptions :column="2" size="small" bordered>
            <a-descriptions-item label="Go inference"
              >{{
                formatRuntimeMs(mlCRuntime.goMsPerSample)
              }}/sample</a-descriptions-item
            >
            <a-descriptions-item label="C inference"
              >{{
                formatRuntimeMs(mlCRuntime.cMsPerSample)
              }}/sample</a-descriptions-item
            >
            <a-descriptions-item label="Speedup">{{
              formatRuntimeSpeedup(mlCRuntime.speedup)
            }}</a-descriptions-item>
            <a-descriptions-item label="Samples">{{
              mlCRuntime.sampleCount || 0
            }}</a-descriptions-item>
            <a-descriptions-item label="Benchmark backend">{{
              runtimeBackendLabel(mlCRuntime.benchmarkBackend)
            }}</a-descriptions-item>
            <a-descriptions-item label="Model">{{
              mlCRuntime.modelType || mlStatus.model_type || "—"
            }}</a-descriptions-item>
          </a-descriptions>
          <div
        ...
        <a-alert
          ...
          type="info"
          show-icon
          message="等待后端返回 C runtime / CUDA / Intel iGPU 状态"
        />
        <a-divider style="margin: 8px 0">Batch Feedback</a-divider>
        <a-input-group compact>
          <a-input
            v-model:value="feedbackComm"
          ...
          <a-button type="dashed" @click="submitFeedback" style="width: 30%"
            >Submit</a-button
          >
        </a-input-group>
      </a-space>
  ...
  <a-col :xs="24">
    <a-card title="Command Safety Assessment" size="small">
      <template #extra
        ><a-tag color="purple">输入完整命令进行安全性判断</a-tag></template
      >
      <a-row :gutter="[16, 16]">
        <a-col :xs="24" :md="8">
          ...
          <div style="margin-top: 12px; font-size: 12px; color: #6b7280">
            ...
            <a
              ...
              :key="i"
              @click="runBacktestPreset(p.comm, p.args)"
          ...
          >
            <div style="display: flex; align-items: center; gap: 16px">
              <div style="flex: 1">
                <div style="font-weight: 600; margin-bottom: 4px">
                  风险评分：{{ backtestResult.riskScore?.toFixed(0) || "-" }} /
                  100
                  <a-tag
                    :color="riskLevelColor(backtestResult.riskLevel)"
                    style="margin-left: 8px"
                    >{{ backtestResult.riskLevel }}</a-tag
                  >
                </div>
                <div
                  style="
              ...
              <div style="text-align: center; min-width: 80px">
                <div
                  style="
                    font-size: 28px;
                    font-weight: bold;
                    color: riskMeterColor(backtestResult.riskScore || 0);
                  "
                >
            ...
            <a-descriptions :column="3" size="small" bordered>
              <a-descriptions-item label="Command">{{
                backtestResult.commandLine || backtestResult.comm
              }}</a-descriptions-item>
              <a-descriptions-item label="Args">{{
                backtestResult.args?.join(" ") || "—"
              }}</a-descriptions-item>
              <a-descriptions-item label="Recommended Action">
                <a-tag
                  :color="
                    backtestResult.recommendedAction === 'BLOCK'
                      ? 'red'
                      : backtestResult.recommendedAction === 'ALERT'
                        ? 'orange'
                        : backtestResult.recommendedAction === 'REWRITE'
                          ? 'blue'
                          : 'green'
                  "
                  >{{ backtestResult.recommendedAction }}</a-tag
                >
              </a-descriptions-item>
              <a-descriptions-item label="Category"
                ><a-tag>{{
                  backtestResult.classification?.primary_category || "UNKNOWN"
                }}</a-tag></a-descriptions-item
              >
              <a-descriptions-item label="Classify Confidence">{{
                backtestResult.classification?.confidence || "—"
              }}</a-descriptions-item>
              <a-descriptions-item label="Anomaly Score">
                <span
                  :style="{
                    color:
                      (backtestResult.anomalyScore ?? 0) > 0.7
                        ? '#d4380d'
                        : (backtestResult.anomalyScore ?? 0) > 0.3
                          ? '#d48806'
                          : '#52c41a',
                  }"
                  >{{ backtestResult.anomalyScore?.toFixed(3) || "—" }}</span
                >
              </a-descriptions-item>
              <a-descriptions-item label="ML Action">{{
                backtestResult.mlPrediction?.action || "—"
              }}</a-descriptions-item>
              <a-descriptions-item label="ML Confidence">{{
                backtestResult.mlPrediction?.confidence
                  ? (backtestResult.mlPrediction.confidence * 100).toFixed(0) +
                    "%"
                  : "—"
              }}</a-descriptions-item>
              <a-descriptions-item label="Reasoning" :span="3">{{
            ...
            <div v-if="backtestResult.llmAssessment" style="margin-top: 8px">
              ...
              >
                <span>LLM 打分结果</span>
                <a-tag
                  :color="backtestResult.llmAssessment.error ? 'red' : 'purple'"
                  >{{
                    backtestResult.llmAssessment.error
                      ? "Error"
                      : "OpenAI-style"
                  }}</a-tag
                >
              ...
              <a-descriptions v-else :column="3" size="small" bordered>
                <a-descriptions-item label="Model">{{
                  backtestResult.llmAssessment.model || "—"
                }}</a-descriptions-item>
                <a-descriptions-item label="Risk Score">{{
                  backtestResult.llmAssessment.riskScore?.toFixed(0) || "—"
                }}</a-descriptions-item>
                <a-descriptions-item label="Confidence">{{
                  backtestResult.llmAssessment.confidence
                    ? (backtestResult.llmAssessment.confidence * 100).toFixed(
                        0,
                      ) + "%"
                    : "—"
                }}</a-descriptions-item>
                <a-descriptions-item label="Recommended Action">
                  <a-tag
                    :color="
                      backtestResult.llmAssessment.recommendedAction === 'BLOCK'
                        ? 'red'
                        : backtestResult.llmAssessment.recommendedAction ===
                          ...
                          : backtestResult.llmAssessment.recommendedAction ===
                            ...
                            : 'green'
                    "
                    >{{ backtestResult.llmAssessment.recommendedAction }}</a-tag
                  >
                </a-descriptions-item>
                <a-descriptions-item label="Reasoning" :span="2">{{
                  backtestResult.llmAssessment.reasoning || "—"
                }}</a-descriptions-item>
                <a-descriptions-item label="Signals" :span="3">
                  <a-space wrap>
                    <a-tag
                      v-for="(signal, i) in backtestResult.llmAssessment
                        .signals || []"
                      :key="i"
                      color="purple"
                      >{{ signal }}</a-tag
            ...
            <div v-if="(backtestResult.sampleEvidence?.totalMatches ?? 0) > 0">
              <a-alert
                show-icon
                :type="
                  backtestResult.sampleEvidence?.decision === 'BLOCK'
                    ? 'error'
                    : backtestResult.sampleEvidence?.decision === 'ALERT'
                      ? 'warning'
                      : 'info'
                "
            ...
            >
              ...
              >
                ...
                <a-tag
                  :color="
                    backtestResult.networkAudit.riskLevel === 'CRITICAL'
                      ? 'red'
                      : backtestResult.networkAudit.riskLevel === 'HIGH'
                        ? 'orange'
                        : backtestResult.networkAudit.riskLevel === 'MEDIUM'
                          ? 'gold'
                          : 'blue'
                  "
              ...
              <a-list
                size="small"
                bordered
                :data-source="backtestResult.networkAudit.findings"
              >
                      ...
                      <template #title>
                        ...
                        >
                          <a-tag
                            ...
                            "
                            size="small"
                            >{{ item.severity.toUpperCase() }}</a-tag
                          >
                          <span>{{ item.type }}</span>
                        </span>
                      </template>
