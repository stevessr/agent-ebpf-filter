...
const gnnNodes: GnnNode[] = [
  {
    ...
    details: "包括未识别(UNKNOWN=0)、文件读取(FILE_READ=1)、文件写入(FILE_WRITE=2)、文件删除(FILE_DELETE=3)、文件权限(FILE_PERMISSION=4)、网络通信(NETWORK=5)、进程启动(PROCESS_EXEC=6)、进程杀死(PROCESS_KILL=7)、系统元数据(SYSTEM_INFO=8)、包管理器(PACKAGE_MANAGER=9)、数据库操作(DATABASE=10)、压缩归档(COMPRESSION=11)、开发构建(DEVELOPMENT=12)、容器编排(CONTAINER=13)、敏感提权(SENSITIVE=14)等 15 维一热特征表示。"
  },
  {
    id: 1,
    name: "process_flags",
    ...
    details: "指示是否为 Shell 终端(is_shell)、包管理器(is_package_manager)、系统 Agent CLI(is_agent_cli)、以及是否以 root 权限运行(is_root_user)、命令行是否携带网络相关参数(has_network_args)、路径相关参数(has_file_args)等特征。"
  },
  {
    id: 2,
    name: "io_flags",
  ...
  },
  {
    ...
    details: "包含命令名字符长度、命令行参数(args)个数等经过归一化限制在[0, 1]区间的连续数值特征。"
  },
  {
    id: 4,
    name: "arg_statistics",
  ...
  },
  {
    ...
    details: "局部敏感哈希(LSH)指令语义特征物化降维向量中第 17 至 32 维，与低维分量联合表示操作的高维上下文语义。"
  },
  {
    id: 10,
    name: "freq_history",
  ...
  return [
    {
      ...
      features: [
        { index: 0, name: "behavior_category[UNKNOWN]", desc: "未识别/默认系统行为" },
        { index: 1, name: "behavior_category[FILE_READ]", desc: "文件读取操作行为 (如 cat, less, hexdump)" },
        { index: 2, name: "behavior_category[FILE_WRITE]", desc: "文件写入/修改操作行为 (如 cp, tee, touch)" },
        { index: 3, name: "behavior_category[FILE_DELETE]", desc: "文件删除/擦除操作行为 (如 rm, shred, unlink)" },
        { index: 4, name: "behavior_category[FILE_PERMISSION]", desc: "更改权限或属主/ACL操作行为 (如 chmod, chown)" },
        { index: 5, name: "behavior_category[NETWORK]", desc: "网络发起连接/请求行为 (如 curl, wget, nc, tcpdump)" },
        { index: 6, name: "behavior_category[PROCESS_EXEC]", desc: "进程启动或 Shell 命令行执行 (如 exec, bash, sh)" },
        { index: 7, name: "behavior_category[PROCESS_KILL]", desc: "进程强制杀死/终止行为 (如 kill, killall, pkill)" },
        { index: 8, name: "behavior_category[SYSTEM_INFO]", desc: "主机指标/元数据查询操作 (如 uptime, df, env, ss)" },
        { index: 9, name: "behavior_category[PACKAGE_MANAGER]", desc: "包管理器安装/卸载软件行为 (如 apt, npm, pip, gem)" },
        { index: 10, name: "behavior_category[DATABASE]", desc: "数据库控制台访问或备份行为 (如 mysql, redis-cli)" },
        { index: 11, name: "behavior_category[COMPRESSION]", desc: "文件解压缩或打包归档行为 (如 tar, zip, gzip)" },
        { index: 12, name: "behavior_category[DEVELOPMENT]", desc: "软件编译构建与版本管理操作 (如 git, gcc, node)" },
        { index: 13, name: "behavior_category[CONTAINER]", desc: "虚拟化容器与集群管理行为 (如 docker, kubectl)" },
        { index: 14, name: "behavior_category[SENSITIVE]", desc: "高危管理/提权/内核模块操作 (如 sudo, passwd, modprobe)" },
        { index: 15, name: "is_shell", desc: "判定执行程序文件名是否为常用终端 Shell 解释器" },
        { index: 16, name: "is_package_manager", desc: "判定执行程序是否为包管理器或软件安装运行程序" },
        { index: 17, name: "is_agent_cli", desc: "判断该命令是否来源于 Agent 本身的管理命令或包装命令行" },
        { index: 18, name: "is_root_user", desc: "判定命令发起者是否为超级管理员 root (UID=0)" },
        { index: 19, name: "has_network_args", desc: "命令行参数中检测到 -p/--port/http/IP 等网络配置选项" },
        { index: 20, name: "has_file_args", desc: "命令行参数中检测到路径格式的字符串 (如 /etc/passwd 等)" },
        ...
        { index: 24, name: "dev_access", desc: "命令行中是否读取 /dev/ 路径 (涉及直连硬件或特殊设备)" },
        { index: 25, name: "sudo_in_args", desc: "参数中显式包含 sudo 提权命令" },
    ...
    },
    {
      ...
      features: [
        ...
        { index: 37, name: "pos_count_ratio", desc: "位置参数(Positional args)在总参数中的占比" },
        ...
        { index: 60, name: "redirect_count", desc: "命令行重定向操作符次数统计" },
    ...
    },
    {
      ...
      dim: 32,
      color: "purple",
      description: "通过局部敏感哈希(LSH)在语义空间中对整条命令行进行语义降维哈希投影所得的 32 维特征向量",
    ...
    },
    {
      ...
      features: [
        ...
        { index: 116, name: "sin_hour", desc: "小时的 Sine 投射，解决跨天交界坐标轴脱节问题" },
        { index: 117, name: "cos_hour", desc: "小时的 Cosine 投射，与正弦投射结合进行二维周期表示" },
        { index: 118, name: "is_weekend", desc: "判定运行时间是否处于周六或周日 (非工作时间访问)" },
    ...
    },
    {
      ...
      features: [
        ...
        { index: 124, name: "dns_tunnel_flag", desc: "检测到目的域名长度及频率极度异常，疑似走 DNS 掩护隧道" },
        { index: 125, name: "cleartext_proto_flag", desc: "检测到使用了易被嗅探的明文通信协议 (如 FTP/Telnet/Plain HTTP)" },
        { index: 126, name: "unusual_target_flag", desc: "目标主机 IP 处于地理黑名单或从未交互过的陌生外部目标" },
          ...
          <template #description>
            <div class="pipeline-desc">
              ...
              <div v-else style="margin-top: 4px">
                在当前选中的 <b>{{ modelTypeLabel }}</b> 模型中，特征向量保持 128 维平铺（Flat）格式输入分类器中，由决策森林、线性 SVM、朴素贝叶斯或集成表决器判定安全概率。
              </div>
            </div>
          </template>
        </a-alert>
