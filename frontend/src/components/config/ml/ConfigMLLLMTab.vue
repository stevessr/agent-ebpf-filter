...
const productionPreviewRowKey = (record: any) =>
...
<template>
  <a-col :xs="24" :md="12">
    <a-card title="LLM Scoring" size="small">
      <template #extra>
        <a-space>
          <a-tag v-if="llmSaveStatus === 'saving'" color="processing"
            ><ReloadOutlined spin /> Saving...</a-tag
          >
          <a-tag v-else-if="llmSaveStatus === 'saved'" color="success"
            ><CheckCircleOutlined /> Saved</a-tag
          >
          <a-tag v-else-if="llmSaveStatus === 'error'" color="error"
            ><ExclamationCircleOutlined /> Save Failed</a-tag
          >
      ...
      <a-space direction="vertical" style="width: 100%">
        ...
        <a-row :gutter="[12, 12]">
          <a-col :xs="24">
            <a-space align="center" wrap>
              ...
              <span>启用 LLM 打分</span>
              <a-tag :color="llmApiKeyStatus.color">{{
                llmApiKeyStatus.text
              }}</a-tag>
            </a-space>
          </a-col>
          <a-col :xs="24">
            ...
            <a-input
              v-model:value="llmScoringConfig.baseUrl"
          ...
          <a-col :xs="24" :md="12">
            ...
            <a-input
              v-model:value="llmScoringConfig.model"
              placeholder="gpt-4o-mini / gpt-5 / 自建兼容模型"
              allow-clear
            />
          </a-col>
          <a-col :xs="24" :md="12">
            ...
            <a-input-password
              v-model:value="llmScoringConfig.apiKey"
              placeholder="留空则保留后端现有密钥"
              allow-clear
            />
          </a-col>
          <a-col :xs="24" :md="8">
          ...
          <a-col :xs="24" :md="8">
            ...
            <a-input-number
              v-model:value="llmScoringConfig.temperature"
              :min="0"
              :max="2"
              :step="0.1"
              style="width: 100%"
          ...
          <a-col :xs="24" :md="8">
            ...
            <a-input-number
              v-model:value="llmScoringConfig.maxTokens"
              :min="32"
              :max="4096"
              :step="32"
              style="width: 100%"
        ...
        <a-row :gutter="[12, 12]">
          <a-col :xs="24" :md="8">
            ...
            <a-select v-model:value="llmBatchConfig.source" style="width: 100%">
              <a-select-option value="training"
                >生成数据（生成新样本并打分）</a-select-option
              >
              <a-select-option value="validation"
                >已有数据（对现有样本重打分）</a-select-option
              >
            </a-select>
          </a-col>
          <a-col :xs="24" :md="8">
          ...
          <a-col :xs="24">
            <a-space align="center" wrap>
              <a-switch
                v-model:checked="llmBatchConfig.applyLabels"
                :disabled="!llmBatchCanApplyLabels"
              />
              <span>把 LLM 结果回写为训练标签</span>
              <a-tag v-if="!llmBatchCanApplyLabels" color="default"
                >仅训练集可回写</a-tag
              >
            </a-space>
          </a-col>
        ...
        <a-button
          type="primary"
          @click="runLLMBatchScore"
          :loading="llmBatchLoading"
          block
        >
          <ThunderboltOutlined /> 开始批量打分
        </a-button>
        <div
          v-if="llmBatchResponse"
        ...
        >
          <a-alert
            ...
            :description="
              llmBatchResponse.review?.validationSplitRatio !== undefined
                ? `验证集切分比例 ${(llmBatchResponse.review.validationSplitRatio * 100).toFixed(0)}%`
                : 'LLM 批量复核已完成。'
            "
          />
          <a-space wrap>
            ...
            <a-tag color="orange"
              >skipped: {{ llmBatchResponse.skipped }}</a-tag
            >
          </a-space>
          <a-table
          ...
          >
            ...
            <a-table-column title="Label" dataIndex="currentLabel" :width="100">
              <template #default="{ record }"
                ><a-tag :color="getLabelColor(record.currentLabel || '-')">{{
                  record.currentLabel || "—"
                }}</a-tag></template
              >
            </a-table-column>
            ...
            >
              <template #default="{ record }">
                <a-tag
                  :color="
                    record.recommendedAction === 'BLOCK'
                      ? 'red'
                      : record.recommendedAction === 'ALERT'
                        ? 'orange'
                        : record.recommendedAction === 'REWRITE'
                          ? 'blue'
                          : 'green'
                  "
                  >{{ record.recommendedAction }}</a-tag
                >
              </template>
            </a-table-column>
            <a-table-column
  ...
  <a-col :xs="24">
    <a-card title="LLM 生产训练集" size="small">
      <template #extra
        ><a-tag color="green">来源：/config/ml/training</a-tag></template
      >
      <a-space direction="vertical" style="width: 100%">
        <a-alert
          type="info"
        ...
        <a-row :gutter="[12, 12]">
          ...
          <a-col :xs="24" :md="8">
            <a-space direction="vertical" style="width: 100%">
              <a-space align="center" wrap
                ><a-switch v-model:checked="llmProductionDeduplicate" /><span
                  >命令 + 标签去重</span
                ></a-space
              >
              <a-space align="center" wrap
                ><a-switch v-model:checked="llmProductionAllowHeuristic" /><span
                  >允许启发式 / LLM 自动标签</span
                ></a-space
              >
            </a-space>
          ...
          <a-col :xs="24" :md="8">
            <a-space direction="vertical" style="width: 100%">
              <a-button
                type="primary"
                @click="fetchLLMProductionDataset()"
                :loading="llmProductionLoading"
                block
                ><ReloadOutlined /> 拉取当前训练集</a-button
              >
              <a-button
                @click="exportLLMProductionDataset()"
                :disabled="
                  llmProductionPreview.length === 0 || llmProductionLoading
                "
                block
                ><DownloadOutlined /> 导出 JSONL</a-button
              >
            </a-space>
          </a-col>
        </a-row>
        <a-space wrap>
          <a-tag v-if="llmProductionMeta" color="blue"
            >source: {{ llmProductionMeta.source }}</a-tag
          >
          <a-tag v-if="llmProductionMeta" color="cyan"
            >format: {{ llmProductionMeta.format }}</a-tag
          >
          <a-tag v-if="llmProductionMeta" color="geekblue"
            >total: {{ llmProductionMeta.total }}</a-tag
          >
          <a-tag v-if="llmProductionMeta" color="green"
            >included: {{ llmProductionMeta.included }}</a-tag
          >
          <a-tag v-if="llmProductionMeta" color="default"
            >skip unlabeled: {{ llmProductionMeta.skippedUnlabeled }}</a-tag
          >
          <a-tag
            v-if="llmProductionMeta && llmProductionMeta.skippedHeuristic > 0"
            color="orange"
            >skip noisy: {{ llmProductionMeta.skippedHeuristic }}</a-tag
          >
          <a-tag
            v-if="llmProductionMeta && llmProductionMeta.skippedDuplicates > 0"
            color="gold"
            >skip dup: {{ llmProductionMeta.skippedDuplicates }}</a-tag
          >
          <a-tag v-if="llmProductionMeta?.truncated" color="red"
            >truncated</a-tag
          >
        </a-space>
        <a-alert
        ...
        <a-table
          ...
          :scroll="{ x: 1280 }"
          size="small"
          :rowKey="productionPreviewRowKey"
        >
          ...
          <a-table-column title="Label" dataIndex="label" :width="100">
            <template #default="{ record }"
              ><a-tag :color="getLabelColor(record.label)">{{
                record.label
              }}</a-tag></template
            >
          </a-table-column>
          ...
          <a-table-column
            title="Confidence"
            dataIndex="targetConfidence"
            :width="110"
          >
            <template #default="{ record }">{{
              record.targetConfidence
                ? (record.targetConfidence * 100).toFixed(0) + "%"
                : "—"
            }}</template>
          </a-table-column>
          ...
          <a-table-column title="Signals" dataIndex="signals" :width="220">
            <template #default="{ record }">
              <a-space wrap size="small"
                ><a-tag
                  v-for="(signal, i) in record.signals || []"
                  :key="i"
                  color="purple"
                  size="small"
                  >{{ signal }}</a-tag
          ...
          <a-table-column title="Reasoning" dataIndex="reasoning" ellipsis>
            <template #default="{ record }"
              ><span>{{ record.reasoning || "—" }}</span></template
            >
          </a-table-column>
        </a-table>
        <a-card
        ...
        >
          <a-descriptions :column="2" size="small" bordered>
            <a-descriptions-item label="Command">{{
              maskSensitiveData(llmProductionPreview[0].commandLine)
            }}</a-descriptions-item>
            <a-descriptions-item label="Label"
              ><a-tag :color="getLabelColor(llmProductionPreview[0].label)">{{
                llmProductionPreview[0].label
