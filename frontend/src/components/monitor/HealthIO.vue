        ...
        <template #extra>
          <a-space>
            <a-button
              ...
              size="small"
              @click="
                openHistoryChart('Global Network Activity', [
                  {
                    name: 'Recv',
                    data: statsHistory.netRecv,
                    color: '#52c41a',
                  },
                  {
                    name: 'Sent',
                    data: statsHistory.netSent,
                    color: '#1890ff',
                  },
                ])
              "
            ...
            <a-button
              ...
              @click="
                openHistoryChart(
                  'Split Network Activity',
                  Object.entries(statsHistory.netDevices).flatMap(
                    ([name, d]) => [
              ...
              >
                ...
                <div style="display: flex; gap: 16px; font-size: 12px">
                  <span style="color: #52c41a; flex: 1"
                    >↓ {{ formatBytesWithUnit(iface.readSpeed) }}/s</span
                  >
                  <span style="color: #1890ff; flex: 1"
                    >↑ {{ formatBytesWithUnit(iface.writeSpeed) }}/s</span
                  >
                </div>
              </div>
            </div>
        ...
        <template #extra>
          <a-space>
            <a-button
              ...
              size="small"
              @click="
                openHistoryChart('Global Disk Activity', [
                  {
                    name: 'Read',
                    data: statsHistory.diskRead,
                    color: '#faad14',
                  },
                  {
                    name: 'Write',
                    data: statsHistory.diskWrite,
                    color: '#722ed1',
                  },
                ])
              "
            ...
            <a-button
              ...
              @click="
                openHistoryChart(
                  'Split Disk Activity',
                  Object.entries(statsHistory.diskDevices).flatMap(
                    ([name, d]) => [
          ...
          <div v-for="(disk, name) in groupedDiskDevices" :key="name">
            ...
            >
              ...
              <div
                ...
                "
              >
                <span style="color: #faad14; flex: 1; font-weight: 500"
                  >Read: {{ formatBytesWithUnit(disk.main.readSpeed) }}/s</span
                >
                <span style="color: #722ed1; flex: 1; font-weight: 500"
                  >Write:
                  {{ formatBytesWithUnit(disk.main.writeSpeed) }}/s</span
                >
              </div>
              <div v-if="disk.partitions.length > 0">
                <div
                  ...
                  "
                >
                  <span style="width: 80px; font-family: monospace"
                    >└─ {{ part.name }}</span
                  >
                  <span style="color: #faad14; opacity: 0.8"
                    >R: {{ formatBytesWithUnit(part.readSpeed) }}/s</span
                  >
                  <span style="color: #722ed1; opacity: 0.8"
                    >W: {{ formatBytesWithUnit(part.writeSpeed) }}/s</span
                  >
                </div>
              </div>
            </div>
