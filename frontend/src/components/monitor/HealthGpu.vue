...
const tempColor = (t: number) =>
  t > 80 ? "#ff4d4f" : t > 65 ? "#faad14" : "#52c41a";
...
const openGpuUtilHistory = (gpu: GPUStatus) => {
  props.openHistoryChart(`GPU #${gpu.index} Utilization`, [
    {
      name: "GPU Util",
      data: props.statsHistory.gpus[gpu.index]?.util || [],
      color: "#722ed1",
    },
  ]);
};
...
const openGpuVramUtilHistory = (gpu: GPUStatus) => {
  props.openHistoryChart(`GPU #${gpu.index} VRAM Utilization`, [
    {
      name: "VRAM Util",
      data: props.statsHistory.gpus[gpu.index]?.mem || [],
      color: "#13c2c2",
    },
  ]);
};
...
const openGpuVramUsageHistory = (gpu: GPUStatus) => {
  ...
  props.openHistoryChart(`GPU #${gpu.index} VRAM Usage`, [
    ...
    {
      name: "VRAM Total",
      data: history.map((d) => ({ time: d.time, value: totalBytes })),
      color: "#8c8c8c",
    },
  ]);
};
  ...
  props.openHistoryChart(`GPU #${gpu.index} Power Usage`, [
    ...
    ...(limit > 0
      ? [
          {
            name: "Power Limit",
            data: history.map((d) => ({ time: d.time, value: limit })),
            color: "#faad14",
          },
        ]
      : []),
      ...
      >
        ...
        <template #title>
          ...
          >
            ...
            <a-tag
              v-if="isIntegrated(gpu)"
              color="purple"
              style="font-size: 10px"
              >iGPU</a-tag
            >
            <a-tag
              v-if="isNvidia(gpu) && (gpu.pcieGen ?? 0) > 0"
              color="geekblue"
              style="font-size: 10px"
              >PCIe {{ gpu.pcieGen }}.0 x{{ gpu.pcieWidth }}</a-tag
            >
            <a-tag v-if="(gpu.temp ?? 0) > 0" :color="tempColor(gpu.temp)"
              >{{ gpu.temp }}°C</a-tag
            >
          </div>
        </template>
          ...
          <a-col :span="8">
            <div style="display: grid; gap: 8px; padding-top: 4px">
              ...
              <template v-if="!isIntegrated(gpu) && (gpu.memTotal ?? 0) > 0">
                ...
                <a-progress
                  :percent="
                    Math.round(((gpu.memUsed ?? 0) / (gpu.memTotal || 1)) * 100)
                  "
                  size="small"
                  :stroke-color="
                    (gpu.memUsed ?? 0) / (gpu.memTotal || 1) > 0.9
                      ? '#ff4d4f'
                      : '#1890ff'
                  "
                  :show-info="false"
              ...
              <div
                ...
                :style="{ color: tempColor(gpu.temp ?? 0) }"
              >
              ...
              >
                ...
                >
                  ...
                  <template v-if="(gpu.powerLimitW ?? 0) > 0">
                    / {{ gpu.powerLimitW }}W limit</template
                  >
                </div>
                <a-progress
                  :percent="
                    (gpu.powerLimitW ?? 0) > 0
                      ? Math.round(
                          ((gpu.powerW ?? 0) / (gpu.powerLimitW || 1)) * 100,
                        )
                      : 0
                  "
                  size="small"
                  ...
                  :stroke-color="
                    (gpu.powerLimitW ?? 0) > 0 &&
                    (gpu.powerW ?? 0) / (gpu.powerLimitW || 1) > 0.8
                      ? '#faad14'
                      : '#52c41a'
                  "
                  style="cursor: pointer"
              ...
              >
                Fan: {{ gpu.fanSpeed }}%
                <a-progress
                  :percent="gpu.fanSpeed ?? 0"
                  size="small"
                  :show-info="false"
                  :stroke-color="
                    (gpu.fanSpeed ?? 0) > 80 ? '#ff4d4f' : '#1890ff'
                  "
                />
              </div>
            </div>
        ...
        <a-row v-if="isNvidia(gpu)" :gutter="16">
          <!-- Encoder -->
          <a-col
            v-if="(gpu.encUtil ?? 0) >= 0"
            :span="6"
            style="text-align: center"
          >
            <a-progress
              type="circle"
              :percent="Math.round(gpu.encUtil ?? 0)"
              :width="64"
          ...
          <!-- Decoder -->
          <a-col
            v-if="(gpu.decUtil ?? 0) >= 0"
            :span="6"
            style="text-align: center"
          >
            <a-progress
              type="circle"
              :percent="Math.round(gpu.decUtil ?? 0)"
              :width="64"
