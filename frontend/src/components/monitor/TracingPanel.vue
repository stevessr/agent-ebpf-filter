...
const columns = [
  ...
  },
  {
    ...
    width: 90,
    align: "right",
    sorter: (a: any, b: any) => a.mem - b.mem,
  },
  {
    title: "GPU Util",
    dataIndex: "gpuUtil",
    ...
    width: 90,
    align: "right",
    sorter: (a: any, b: any) => a.gpuUtil - b.gpuUtil,
  },
  {
    title: "VRAM",
    dataIndex: "gpuMem",
    ...
    width: 90,
    align: "right",
    sorter: (a: any, b: any) => a.gpuMem - b.gpuMem,
  },
...
<template>
  <div style="background: #fff; padding: 20px; border-radius: 4px">
    ...
    >
      ...
      >
        ...
        <a-tag
          ...
          :key="name"
          :color="
            hiddenComms.includes(name)
              ? undefined
              : activeCommNames.has(name)
                ? 'green'
                : 'blue'
          "
          :style="
            hiddenComms.includes(name)
              ? 'opacity:0.45;text-decoration:line-through;cursor:pointer;'
              : 'cursor:pointer;'
          "
    ...
    <a-table
      ...
      :rowClassName="() => 'tracked-row'"
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'cpu'"
          ><span :style="{ color: (record.cpu ?? 0) > 50 ? 'red' : 'inherit' }"
            >{{ (record.cpu ?? 0).toFixed(1) }}%</span
          ></template
        >
        <template v-if="column.key === 'action'">
          <a-space>
            <a-button
              type="link"
              size="small"
              @click="sendProcessSignal(record.pid, 'stop')"
              >Suspend</a-button
            >
            <a-button
              type="link"
              size="small"
              @click="sendProcessSignal(record.pid, 'cont')"
              >Resume</a-button
            >
            <a-button
              type="link"
              size="small"
              danger
              @click="sendProcessSignal(record.pid, 'kill')"
              >Kill</a-button
            >
          </a-space>
        </template>
