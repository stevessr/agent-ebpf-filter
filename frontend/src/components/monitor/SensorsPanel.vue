...
<template>
  ...
  >
    <a-tabs
      :activeKey="sensorSubTab"
      @change="emit('update:sensorSubTab', $event as string)"
      size="small"
    >
      <a-tab-pane key="hardware" tab="Hardware">
        <div style="display: flex; flex-direction: column; gap: 16px">
          ...
          >
            <a-space>
              ...
              <a-select
                ...
                size="small"
                style="width: 70px"
                @change="emit('update:sensorInterval', $event)"
              >
              ...
              <a-button-group size="small">
                ...
                <a-button @click="emit('toggleAllSensors', false)"
                  >None</a-button
                >
              </a-button-group>
            </a-space>
          ...
          <div
            v-for="(sensors, category) in groupedSensors"
            :key="category"
            ...
            <a-row :gutter="16">
                ...
                >
                  <VueApexCharts
                    ...
                    :options="{
                      ...sensorChartOptions,
                      chart: {
                        ...sensorChartOptions.chart,
                        id: `chart-${category.replace(/\s+/g, '-')}`,
                      },
                    }"
                    :series="
                      (sensors as any[])
                        ...
                        .map((s) => ({
                          ...
                          data: (sensorHistory[s.sensorKey] || []).map((d) => ({
                            x: d.time,
                            y: d.value,
                          })),
                        }))
              ...
              <a-col :span="8">
                ...
                >
                  ...
                  >
                    <a-checkbox
                      :checked="sensorVisibility[s.sensorKey]"
                      @change="
                        (e: any) =>
                          emit(
                    ...
                    >
                      <span
                        style="
                          ...
                          max-width: 150px;
                        "
                        :title="s.label || s.sensorKey"
                        >{{ s.label || s.sensorKey }}</span
                      >
                    </a-checkbox>
                    <span
                      :style="{
                      ...
                      >{{ s.temperature.toFixed(1) }}°C</span
                    >
                  </div>
                </div>
              </a-col>
          ...
          <div v-if="fanData.length > 0">
            ...
            <a-row :gutter="16"
              ><a-col v-for="f in fanData" :key="f.label" :span="6"
                ><a-card size="small" style="margin-bottom: 8px"
                  ><a-statistic
                    :title="f.label"
                    :value="f.speed"
                    suffix="RPM" /></a-card></a-col
            ></a-row>
          </div>
        </div>
      </a-tab-pane>
        ...
        <a-card title="Live Feed" size="small">
          <template #extra
            ><a-space
              ...
              ><span>Live:</span
              ><a-switch
                :checked="cameraLiveMode"
                @change="
                  (checked: boolean) => emit('update:cameraLiveMode', checked)
                "
                size="small" /></a-space
          ></template>
          <div style="display: flex; gap: 16px">
            <div style="flex: 1">
              <a-select
                ...
                @change="
                  emit('update:selectedCamera', $event);
                  emit('refreshCamera');
                "
              >
                <a-select-option
            ...
            <div v-if="!cameraLiveMode" style="width: 200px">
              <a-card size="small" title="Snapshot"
                ><a-button block size="small" @click="emit('refreshCamera')"
                  >Capture</a-button
                ></a-card
              >
            </div>
          ...
          <template #extra
            ><a-space
              ...
              ><a-switch
                :checked="micLiveMode"
                @change="
                  (checked: boolean) => emit('update:micLiveMode', checked)
                "
                size="small" /></a-space
          ></template>
          <div style="display: flex; gap: 24px; align-items: center">
            ...
            <div style="width: 280px">
              <div style="margin-bottom: 16px">
                ...
                >
                  ...
                  <a-button
                    ...
                    @click="emit('update:micListenBrowser', !micListenBrowser)"
                  >
                    <template #icon
                      ><SoundOutlined
                        v-if="micListenBrowser" /><AudioMutedOutlined v-else
                    /></template>
              ...
              <a-select
                ...
                @change="emit('update:selectedMic', $event)"
              >
                <a-select-option
                  v-for="dev in micDevices"
                  :key="dev.id"
                  :value="dev.id"
                  >{{ dev.name }}</a-select-option
                >
              </a-select>
            </div>
          </div>
