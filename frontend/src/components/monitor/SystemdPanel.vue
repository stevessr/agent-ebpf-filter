...
<template>
  ...
  >
    ...
    >
      <a-space>
        <a-radio-group
          :value="systemdScope"
          @change="
            (e: any) => emit('update:systemdScope', e?.target?.value ?? e)
          "
          button-style="solid"
          size="small"
        >
        ...
        <a-input-search
          :value="systemdSearch"
          @update:value="emit('update:systemdSearch', $event)"
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'active'"
          ><a-tag :color="record.active === 'active' ? 'success' : 'default'">{{
            record.active
          }}</a-tag></template
        >
        <template v-else-if="column.key === 'action'">
          <a-space>
            <a-button
              type="link"
              size="small"
              @click="emit('fetchLogs', record.unit)"
              >Logs</a-button
            >
            <a-button
              v-if="record.active !== 'active'"
              type="link"
              size="small"
              @click="emit('control', record.unit, 'start')"
              >Start</a-button
            >
            <a-button
              v-if="record.active === 'active'"
              ...
              size="small"
              danger
              @click="emit('control', record.unit, 'stop')"
              >Stop</a-button
            >
            <a-button
              type="link"
              size="small"
              @click="emit('control', record.unit, 'restart')"
              >Restart</a-button
            >
          </a-space>
        </template>
    ...
    <a-modal
      :open="showLogsModal"
      :title="`Logs: ${activeLogUnit}`"
      width="1000px"
      :footer="null"
      @update:open="emit('update:showLogsModal', $event)"
    >
      <div
        style="
          background: #1e1e1e;
