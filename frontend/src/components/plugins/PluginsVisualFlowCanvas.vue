  ...
  <div class="flow-shell">
    <div class="flow-header">
      <div>
        ...
        <span
          >从左侧拖节点类型到画布会按 24px
          网格吸附；拖节点端口可编辑路由，拖动底部手柄可拉开工作区。</span
        >
      </div>
      <a-space size="small" wrap>
        <a-tag color="blue"
          >{{ canvasSize.width }}×{{ canvasSize.height }}</a-tag
        >
        <a-tag color="geekblue">24px snap</a-tag>
        <a-tag color="cyan">route edit</a-tag>
        <a-tag
          :color="
            connectedWireCount === visibleWireDefinitions.length
              ? 'green'
              : 'orange'
          "
        >
        ...
        >
          ...
          <span
            ...
            :class="{
              ...
              valid:
                connectionDrag?.target === node.id && connectionDrag?.valid,
              invalid:
                connectionDrag?.target === node.id && !connectionDrag?.valid,
            }"
          ></span>
          <span
            ...
            @pointerdown.stop.prevent="
              handleConnectionPointerDown($event, node.id)
            "
          ></span>
