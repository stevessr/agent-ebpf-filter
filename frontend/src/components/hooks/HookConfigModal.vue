...
const currentHookDoc = computed<HookCliDoc | null>(() =>
  getHookCliDoc(props.hook?.id),
);
...
const usedEventNames = computed(
  () => new Set(Object.keys(parsedConfig.value?.hooks || {})),
);
const supportsAsyncCommandHooks = computed(
  () => props.hook?.id !== "codex" && props.hook?.id !== "augment",
);
const supportsTimeoutCommandHooks = computed(
...
const addEvent = () => {
  ...
  if (parsedConfig.value.hooks[name])
    return message.warning(`Event '${name}' already exists`);
...
const getEventDoc = (eventName: string) =>
  getHookEventDoc(props.hook?.id, eventName);
const getEventFields = (eventName: string) =>
  getEventDoc(eventName)?.fields || [];
const getEventNotes = (eventName: string) =>
  getEventDoc(eventName)?.notes || [];
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <a-radio-button value="visual"
          ><FormOutlined /> Visual Editor</a-radio-button
        >
        <a-radio-button value="raw"
          ><CodeOutlined /> Raw {{ configFormat.toUpperCase() }}</a-radio-button
        >
      </a-radio-group>
    ...
    <div
      v-if="editorMode === 'visual' && supportsVisualEditor"
      style="max-height: 60vh; overflow-y: auto; padding: 4px"
    >
      <a-alert
        v-if="currentHookDoc"
        type="info"
      ...
      >
        ...
        <template #description>
          <div style="display: flex; flex-direction: column; gap: 8px">
            ...
            <div
              v-if="currentHookDoc.commonFields?.length"
            ...
            <ul
              v-if="currentHookDoc.notes?.length"
              style="margin: 0; padding-left: 18px"
            >
              <li v-for="note in currentHookDoc.notes" :key="note">
                {{ note }}
              </li>
            </ul>
          </div>
      ...
      <div
        v-for="(matchers, eventName) in parsedConfig.hooks || {}"
        :key="eventName"
        style="
          margin-bottom: 24px;
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              v-if="getEventFields(eventName as string).length"
              style="display: flex; flex-wrap: wrap; gap: 6px"
            >
              <a-tooltip
                v-for="field in getEventFields(eventName as string)"
                :key="field.name"
              >
            ...
            <ul
              v-if="getEventNotes(eventName as string).length"
              style="
                margin: 0;
                padding-left: 18px;
                color: #4b5563;
            ...
            >
              <li
                v-for="note in getEventNotes(eventName as string)"
                :key="note"
              >
                {{ note }}
              </li>
          ...
          <div style="display: flex; gap: 8px">
            <a-button size="small" @click="addMatcher(eventName as string)"
              ><PlusOutlined /> Add Matcher</a-button
            >
            <a-popconfirm
        ...
        <div style="padding: 16px">
          ...
          <div
            v-for="(matcherBlock, mIdx) in matchers"
            :key="mIdx"
            style="
              margin-bottom: 16px;
              padding: 12px;
          ...
          >
            ...
            >
              ...
              <a-input
                v-model:value="matcherBlock.matcher"
                size="small"
                placeholder="Tool name or * for all"
                style="width: 200px"
              />
              <a-popconfirm
            ...
            <div style="margin-left: 20px">
              <div
                v-for="(hookEntry, hIdx) in matcherBlock.hooks"
                :key="hIdx"
                style="
                  background: #fdfdfd;
                  padding: 12px;
              ...
              >
                <div style="display: flex; gap: 8px; align-items: center">
                  ...
                  <a-input
                    v-model:value="hookEntry.command"
                    size="small"
                ...
                <div style="display: flex; gap: 8px; align-items: center">
                  ...
                  <a-input
                    v-model:value="hookEntry.statusMessage"
                    size="small"
                  ...
                  <template v-if="supportsTimeoutCommandHooks">
                    ...
                    >
                    <a-input-number
                      v-model:value="hookEntry.timeout"
                      size="small"
                      :min="0"
                      :step="1000"
                      style="width: 100px"
              ...
              <a-button
                ...
                type="dashed"
                block
                @click="addCommandHook(eventName as string, Number(mIdx))"
              >
      ...
      >
        ...
        >
          ...
          <a-button
            ...
            size="small"
            @click="addEvent"
            :disabled="!newEventName || usedEventNames.has(newEventName)"
          >
            <PlusOutlined /> Add Hook Event
          </a-button>
          <span
            v-if="newEventName && usedEventNames.has(newEventName)"
        ...
        >
          ...
          <div
            v-if="selectedEventDoc.fields?.length"
            style="display: flex; flex-wrap: wrap; gap: 6px"
          >
            <a-tooltip
              v-for="field in selectedEventDoc.fields"
              :key="field.name"
            >
              <template #title>{{ field.description }}</template>
              <a-tag color="processing">{{ formatFieldLabel(field) }}</a-tag>
