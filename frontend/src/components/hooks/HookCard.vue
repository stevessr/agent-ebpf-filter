...
const docSourceUrl = computed(
  () => getHookCliDoc(props.hook.id)?.sources?.[0]?.url || "",
);
...
<template>
  <a-card size="small" hoverable style="border-radius: 6px">
    ...
    >
      ...
      >
        <a-tag :color="hook.installed ? 'success' : 'default'">
          <template #icon
            ><CheckCircleOutlined v-if="hook.installed"
          /></template>
    ...
    <div
      v-if="hook.hook_type === 'native' && !hook.installed"
