...
import {
  useShellSessions,
  SHELL_MODE_OPTIONS,
} from "../../composables/executor/useShellSessions";
...
const doCreateSession = () => {
  void createSession(props.defaultEnv);
};
  ...
  <div class="shell-manager">
    <a-row :gutter="[16, 16]">
        ...
        <a-card :title="managerTitle" :bordered="false">
          <template #extra>
            <a-space :size="8">
              ...
              <a-button size="small" @click="cleanupSessions"
                >Cleanup Sessions</a-button
              >
              <a-button
                size="small"
            ...
            <a-form layout="vertical">
                ...
                <a-col :span="10">
                  <a-form-item label="Create">
                    <a-button
                      ...
                      >New Session</a-button
                    >
                  </a-form-item>
                </a-col>
              </a-row>
              <a-form-item
                v-if="defaultShellMode === 'custom'"
              ...
              <div class="shell-manager__summary">
                ...
                <span class="shell-manager__summary-text"
                  >New sessions will be created with the selected shell and then
                  attached in a tab.</span
                >
              </div>
            </a-form>
          </template>
          ...
          <a-divider
            v-if="
              showCreatePanel || isTmuxFilteredView || isNonTmuxFilteredView
            "
          />
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.dataIndex === 'session'">
                <div
                  class="shell-manager__session-cell"
                  :title="`${sessionLabel(record)} → ${record.shellPath || 'unresolved'}\n${record.workDir}`"
                >
                  <div class="shell-manager__session-title">
                    #{{ record.id }}
                  </div>
                  <div class="shell-manager__session-badges">
                    <a-tag v-if="isTmuxSession(record)" color="purple"
                      >tmux</a-tag
                    >
                    <a-tag
                      v-else-if="record.kind && record.kind !== 'shell'"
                      color="blue"
                      >{{ record.kind }}</a-tag
                    >
                  </div>
                  <div
                    class="shell-manager__session-subtitle shell-manager__session-subtitle--ellipsis"
                  >
                    {{ sessionLabel(record) }} →
                    {{ record.shellPath || "unresolved" }}
                  </div>
                  <div
                    class="shell-manager__session-subtitle shell-manager__session-subtitle--ellipsis"
                  >
              ...
              <template v-else-if="column.dataIndex === 'pid'"
                ><code>{{ record.pid }}</code></template
              >
              <template v-else-if="column.dataIndex === 'status'">
                <a-space wrap :size="6">
                  <a-tag :color="shellStatusColor(record.status)">{{
                    record.status
                  }}</a-tag>
                ...
                <div class="shell-manager__actions">
                  <a-space wrap :size="8">
                    <a-button
                      ...
                      type="primary"
                      :disabled="
                        !isSessionOpen(record.id) &&
                        (record.status !== 'running' || record.attached)
                      "
                    ...
                    <a-button
                      size="small"
                      danger
                      @click="closeBackendSession(record.id)"
                      >Close</a-button
                    >
                  </a-space>
                  <a-space
      ...
      <a-col :xs="24" :xxl="14">
        <a-card
          :title="
            isTmuxFilteredView ? 'Active Tmux Tabs' : 'Active Terminal Tabs'
          "
          :bordered="false"
        >
          <template #extra>
