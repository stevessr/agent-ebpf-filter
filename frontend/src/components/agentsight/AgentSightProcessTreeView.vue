...
const filteredTree = computed(() =>
  filterProcessTree(processTree.value, filters.value),
);
...
const hasActiveFilters = computed(
  () =>
    ...
    filters.value.commands.length > 0 ||
    filters.value.searchText.length > 0 ||
    Boolean(filters.value.timeRange.start || filters.value.timeRange.end),
);

const eventTypeOptions = computed(() =>
  filterOptions.value.eventTypes.map((value) => ({ label: value, value })),
);
const modelOptions = computed(() =>
  filterOptions.value.models.map((value) => ({ label: value, value })),
);
const sourceOptions = computed(() =>
  filterOptions.value.sources.map((value) => ({ label: value, value })),
);
const commandOptions = computed(() =>
  filterOptions.value.commands.map((value) => ({ label: value, value })),
);
...
const collectProcessPids = (nodes: AgentSightProcessNodeType[]): number[] =>
  nodes.flatMap((node) => [node.pid, ...collectProcessPids(node.children)]);
...
const expandCurrentEvents = () => {
  expandedEvents.value = new Set(
    filteredTree.value.flatMap(function collect(
      node: AgentSightProcessNodeType,
    ): string[] {
      return [
        ...node.events.map((event) => event.id),
...
const updateFilter = <K extends keyof AgentSightProcessFilters>(
  key: K,
  value: AgentSightProcessFilters[K],
) => {
...
const normalizePickerText = (text: string | string[]) =>
...
const setStartTime = (value: string) => {
  updateFilter("timeRange", {
    ...
    start: value ? new Date(value).getTime() : undefined,
  });
};
...
const setEndTime = (value: string) => {
  updateFilter("timeRange", {
    ...filters.value.timeRange,
    end: value ? new Date(value).getTime() : undefined,
  });
};

const onStartPickerChange = (_date: unknown, text: string | string[]) =>
  setStartTime(normalizePickerText(text));
const onEndPickerChange = (_date: unknown, text: string | string[]) =>
  setEndTime(normalizePickerText(text));
const onEventTypesChange = (value: unknown) =>
  updateFilter("eventTypes", normalizeStringArray(value));
const onModelsChange = (value: unknown) =>
  updateFilter("models", normalizeStringArray(value));
const onSourcesChange = (value: unknown) =>
  updateFilter("sources", normalizeStringArray(value));
const onCommandsChange = (value: unknown) =>
  updateFilter("commands", normalizeStringArray(value));
</script>
  ...
  <div class="process-tree-view">
    <div class="tree-header">
      <div>
        <h3>Process Tree</h3>
        <p>
          Hierarchical AgentSight process relationships with prompt, response,
          TLS, file, stdio, policy, and process blocks.
        </p>
      </div>
      <a-space wrap>
        <a-tag color="blue"
          >{{ filteredEvents }} / {{ totalEvents }} events</a-tag
        >
        ...
        <a-button size="small" @click="expandCurrentEvents"
          >Expand events</a-button
        >
        <a-button size="small" @click="collapseAllProcesses">Collapse</a-button>
        <a-button v-if="hasActiveFilters" size="small" @click="clearFilters"
          >Clear</a-button
        >
      </a-space>
    </div>
    ...
    <div class="preset-row">
      <a-button size="small" @click="setPreset(['prompt', 'response'])"
        >AI only</a-button
      >
      <a-button size="small" @click="setPreset(['file'])">Files only</a-button>
      <a-button size="small" @click="setPreset(['process'])"
        >Processes only</a-button
      >
      <a-button size="small" @click="setPreset(['stdio'])"
        >Stdio/MCP only</a-button
      >
      <a-button size="small" @click="setPreset(['ssl'])"
        >TLS/HTTP only</a-button
      >
    </div>
      ...
      <a-row :gutter="[12, 12]">
        <a-col :xs="24" :md="12">
          <a-input
            ...
            placeholder="Search in titles, payloads, commands, models"
            allow-clear
        ...
        <a-col :xs="24" :md="12">
          ...
          <a-select
            :value="filters.commands"
            mode="multiple"
            allow-clear
            show-search
            :options="commandOptions"
            style="width: 100%"
            @change="onCommandsChange"
          />
        </a-col>
      </a-row>
    </a-card>
