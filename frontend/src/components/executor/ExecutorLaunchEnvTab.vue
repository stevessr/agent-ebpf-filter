...
<template>
  <div class="executor-launch-env-tab">
        ...
        >
          <template #extra>
            <a-button type="primary" size="small" @click="addNewProfile"
              ><template #icon><PlusOutlined /></template>New Profile</a-button
            >
          </template>
            ...
            <template v-for="profile in profiles" :key="profile.id">
              <a-card-grid
                :style="{
                  ...
                  borderRadius: '4px',
                  background:
                    activeProfileId === profile.id ? '#e6f7ff' : '#fff',
                }"
    ...
    <a-row :gutter="[16, 16]">
      <a-col :xs="24" :xl="11">
        <a-card
          :title="`Variables in: ${activeProfile.name}`"
          :bordered="false"
        >
          <template #extra>
            <a-space :size="8">
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'action'">
                <a-button
                  size="small"
                  danger
                  @click="removeLaunchEnvEntry(record.id)"
          ...
          <template #extra
            ><a-space :size="8"
              ><SettingOutlined /><span
                >Local browser persistence</span
              ></a-space
            ></template
          >
          <a-space direction="vertical" :size="12" style="width: 100%">
            <a-descriptions bordered size="small" :column="1">
              <a-descriptions-item label="Active variables"
                ><span>{{ launchEnvEntriesCount }}</span></a-descriptions-item
              >
              <a-descriptions-item label="Scope"
                ><span>{{ launchEnvScope }}</span></a-descriptions-item
              >
              <a-descriptions-item label="Preview"
                ><span>{{ launchEnvPreview }}</span></a-descriptions-item
              >
            </a-descriptions>
            <a-alert
              type="warning"
      ...
      <a-col :span="24">
        <a-card title="Detected environment from backend" :bordered="false">
          <template #extra>
            <a-space :size="8">
              <a-tag color="blue"
                >{{ filteredDetectedLaunchEnvEntries.length }} visible</a-tag
              >
              <a-tag color="default"
                >{{ detectedLaunchEnvEntries.length }} detected</a-tag
              >
              <a-button
                size="small"
                :loading="detectedLaunchEnvLoading"
                @click="refreshDetectedLaunchEnvEntries"
                ><template #icon><ReloadOutlined /></template>Refresh</a-button
              >
              <a-button
                size="small"
                type="primary"
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'action'">
                <a-space>
                  <a-tag v-if="isLaunchEnvImported(record.key)" color="green"
                    >Imported</a-tag
                  >
                  <a-button
                    size="small"
                    @click="importDetectedLaunchEnvEntry(record)"
                    >{{
  ...
  >
    <a-form layout="vertical">
      <a-form-item label="Profile Name">
        <a-input
          v-model:value="profileRenameValue"
          placeholder="Enter profile name"
          @pressEnter="applyRename"
        />
      </a-form-item>
    </a-form>
  </a-modal>
