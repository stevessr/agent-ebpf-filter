...
export const CODING_PRESET_OPTIONS: Array<{
  label: string;
  value: CodingPresetKey;
  command: string;
}> = [
...
export function useCodingLauncher(
  ...
  const getSelectedCodingCommand = () => {
    ...
    return (
      CODING_PRESET_OPTIONS.find(
        (option) => option.value === codingPreset.value,
      )?.command || ""
    );
  };
  ...
  const launchCodingCli = async () => {
    ...
    try {
      ...
      await createSession(
        payload,
        `Launched coding CLI: ${cliCommand}`,
        codingUseTmux.value ? "tmux" : "shell",
      );
    } catch (err: any) {
      message.error(
        err?.response?.data?.error ||
          err?.message ||
          "Failed to launch coding CLI",
      );
    } finally {
