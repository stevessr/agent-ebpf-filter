...
function loadProfiles(): LaunchEnvProfile[] {
  try {
    const raw = JSON.parse(
      localStorage.getItem(LAUNCH_ENV_STORAGE_KEY) || "null",
    ) as unknown;
    if (Array.isArray(raw) && raw.length > 0) {
      return raw as LaunchEnvProfile[];
    }
    const legacy = JSON.parse(
      localStorage.getItem(LAUNCH_ENV_LEGACY_KEY) || "[]",
    ) as LaunchEnvEntry[];
...
export function useLaunchEnv() {
  ...
  const activeProfileId = ref<string>(
    localStorage.getItem("executor_active_profile_id") ||
      profiles.value[0]?.id ||
      "",
  );
  ...
  function persistProfiles() {
    localStorage.setItem(
      LAUNCH_ENV_STORAGE_KEY,
      JSON.stringify(profiles.value),
    );
  ...
  const isValidLaunchEnvKey = (key: string) =>
    /^[A-Za-z_][A-Za-z0-9_]*$/.test(key);

  const launchEnvEntriesCount = computed(
    () =>
      launchEnvEntries.value.filter(
        (e) => e.enabled && isValidLaunchEnvKey(e.key.trim()),
      ).length,
  );
  ...
  const launchEnvPreview = computed(() => {
    ...
    return entries.length
      ...
      : "No launch environment overrides configured";
  });

  const launchEnvScope = computed(() => {
    const count = launchEnvEntriesCount.value;
    return count
      ? `${count} active variable${count === 1 ? "" : "s"} applied to Remote, Shell, Tmux, and Script launches`
      : "Applies to all Executor launches";
  });
  ...
  const filteredDetectedLaunchEnvEntries = computed(() => {
    ...
    return detectedLaunchEnvEntries.value.filter(
      (e) =>
  ...
  const addLaunchEnvEntry = () => {
    ...
    if (!isValidLaunchEnvKey(key)) {
      message.error(
        "Environment variable names should look like FOO or FOO_BAR",
      );
      return;
    }
    launchEnvEntries.value = [
  ...
  const isLaunchEnvImported = (key: string) =>
    launchEnvEntryKeys.value.has(key.trim());
  ...
  const importDetectedLaunchEnvEntry = (entry: DetectedLaunchEnvEntry) => {
    ...
    const index = launchEnvEntries.value.findIndex(
    ...
    if (index >= 0) {
      ...
      launchEnvEntries.value = [
        ...
        { ...current, key, value: entry.value, enabled: true },
  ...
  const refreshDetectedLaunchEnvEntries = async () => {
    ...
    try {
      ...
      const items = Array.isArray(res.data?.items)
        ? (res.data.items as DetectedLaunchEnvEntry[])
        : [];
      ...
      detectedLaunchEnvError.value =
        ...
        err?.message ||
        "Failed to load detected env vars";
    } finally {
  ...
  return {
    ...
    clearDisabledLaunchEnvEntries,
    addNewProfile,
    copyProfile,
    deleteProfile,
    openRenameModal,
    detectedLaunchEnvEntries,
    detectedLaunchEnvSearch,
    detectedLaunchEnvLoading,
    detectedLaunchEnvError,
    filteredDetectedLaunchEnvEntries,
    detectedLaunchEnvColumns,
    refreshDetectedLaunchEnvEntries,
    importAllDetectedLaunchEnvEntries,
    isLaunchEnvImported,
    importDetectedLaunchEnvEntry,
    profileRenameModalOpen,
    profileRenameValue,
    applyRename,
  };
}
