...
export function useExecutionGraph(options: ExecutionGraphComposableOptions) {
  ...
  const timePresetLabels: Record<
    ExecutionGraphFilterState["timePreset"],
    string
  > = {
  ...
  const selectedNode = computed(
    () => nodeMap.value.get(selectedNodeId.value) ?? null,
  );
  const selectedNodeKindColor = computed(
    () => kindTagColorMap[selectedNode.value?.kind ?? ""] ?? "default",
  );
  ...
  const relatedNodes = computed(() => {
    ...
    const maxDepth =
      selectedNode.value.kind === "agent_run" ||
      selectedNode.value.kind === "tool_call"
        ? 3
        : 2;
    const relatedIds = collectReachableIds(selectedNode.value.id, maxDepth);
    return graph.value.nodes.filter(
      (node) => relatedIds.has(node.id) && node.id !== selectedNode.value?.id,
    );
  });

  const relatedProcesses = computed(() =>
    relatedNodes.value.filter((node) => node.kind === "process"),
  );
  const relatedFiles = computed(() =>
    relatedNodes.value.filter((node) => node.kind === "file"),
  );
  const relatedNetwork = computed(() =>
    relatedNodes.value.filter((node) => node.kind === "network"),
  );
  const relatedPolicies = computed(() =>
    relatedNodes.value.filter(
      (node) => node.kind === "policy_alert" || node.kind === "policy_decision",
    ),
  );

  const sortedNodeCounts = computed(() =>
    Object.entries(graph.value.nodeCounts ?? {}).sort((a, b) => b[1] - a[1]),
  );
  const sortedEdgeCounts = computed(() =>
    Object.entries(graph.value.edgeCounts ?? {}).sort((a, b) => b[1] - a[1]),
  );
  const metadataEntries = computed(() =>
    Object.entries(selectedNode.value?.metadata ?? {}).filter(
      ([, value]) => value !== "",
    ),
  );
  const processList = computed(() =>
    [...processes.value].sort(
      (a, b) => (b.cpu ?? 0) - (a.cpu ?? 0) || a.pid - b.pid,
    ),
  );
  const processTreeEdgeKinds = new Set([
    "child_process",
    "parent_process",
    "exec_chain",
    "spawned",
  ...
  const selectedProcess = computed(() => {
    ...
    return (
      processList.value.find(
        (process) => process.pid === selectedProcessPid.value,
      ) ?? null
    );
  });
  ...
  const selectedProcessSummary = computed(() => {
    ...
    if (!process) {
      return filters.pid
        ? `Listening to pid ${filters.pid}; it may have exited from the live process list.`
        : "Pick a live process to focus its current and descendant execution graph.";
    }
  ...
  const browserRecordingSummary = computed(() => {
    if (!browserSnapshotCount.value)
      return "浏览器内存尚无录制快照，刷新页面后会丢失。";
    const first = browserSnapshots.value[0]?.recordedAt ?? "";
    const last =
      browserSnapshots.value[browserSnapshots.value.length - 1]?.recordedAt ??
      "";
  ...
  const processTreeNodeIds = computed(() => {
    ...
    graph.value.edges.forEach((edge) => {
      ...
      if (
        processTreeEdgeKinds.has(edge.kind) &&
        source?.kind === "process" &&
        target?.kind === "process"
      ) {
        ids.add(edge.source);
  ...
  const processTreeNodes = computed(() =>
    graph.value.nodes.filter(
      (node) =>
        node.kind === "process" && processTreeNodeIds.value.has(node.id),
    ),
  );
  const processTreeEdges = computed(() =>
  ...
  const buildPresetSince = (
    preset: ExecutionGraphFilterState["timePreset"],
  ) => {
    const now = Date.now();
    switch (preset) {
      case "15m":
  ...
  const normalizeGraphResponse = (
    ...
    edges: Array.isArray(payload?.edges)
      ? (payload!.edges as ExecutionGraphEdge[])
      : [],
  });
