...
export type ExecutionGraphRecordingDeps = {
  ...
  closeGraphSocket: (status?: 'connecting' | 'connected' | 'paused' | 'closed' | 'error') => void;
...
export function useExecutionGraphRecording(deps: ExecutionGraphRecordingDeps) {
  ...
  const playRecording = async () => {
    ...
    try {
      const { data } = await axios.post('/events/recording/replay', { path, limit: deps.filters.limit });
  ...
  const stopBrowserRecording = () => {
    ...
    message.success(`已停止内存录制，共 ${deps.browserSnapshotCount.value} 个快照`);
  };
  ...
  const playBrowserRecording = () => {
    ...
    const playNext = () => {
      ...
      deps.applyGraphPayload({ ...deps.cloneGraphState(snapshot.graph), source: 'browser_memory' });
