...
export interface StatsHistory {
  ...
  netDevices: Record<
    string,
    {
      recv: { time: number; value: number }[];
      sent: { time: number; value: number }[];
    }
  >;
  diskDevices: Record<
    string,
    {
      read: { time: number; value: number }[];
      write: { time: number; value: number }[];
    }
  >;
}
...
export function useMonitorData() {
  ...
  const byteScale = (bytes: number): ByteScale => {
    ...
    let divisor = 1;
    let i = 0;
    while (bytes >= divisor * 1024 && i < units.length - 1) {
      divisor *= 1024;
      i++;
    }
  ...
  const groupedNetInterfaces = computed(() => {
    const groups: Record<string, IOSpeed[]> = {
      Loopback: [],
      "Wi-Fi": [],
      Ethernet: [],
      Virtual: [],
  ...
  const groupedDiskDevices = computed(() => {
    ...
    systemStats.value.diskDevices.forEach((d) => {
      const isPartition =
        /p\d+$/.test(d.name) ||
        (/[a-z]\d+$/.test(d.name) && !d.name.startsWith("nvme"));
    ...
    systemStats.value.diskDevices.forEach((d) => {
      const isPartition =
        ...
        (/[a-z]\d+$/.test(d.name) && !d.name.startsWith("nvme"));
      if (isPartition) {
        let parent = d.name.startsWith("nvme")
          ? d.name.split("p")[0]
          : d.name.replace(/\d+$/, "");
  ...
  const topFaultProcesses = computed(() => {
    if (mergeFaultProcesses.value) {
      ...
      return Object.values(merged)
        .sort(
          (a, b) =>
            b.majorFaults + b.minorFaults - (a.majorFaults + a.minorFaults),
        )
  ...
  const trackedProcesses = computed(() => {
    ...
    return processes.value.filter((p) =>
      trackedCommsNames.value.includes(p.name),
    );
  });

  const getCoreTypeColor = (type: number) =>
    type === pb.CPUInfo.Core.Type.PERFORMANCE ? "#1890ff" : "#52c41a";
  const getCoreTypeName = (type: number) =>
    type === pb.CPUInfo.Core.Type.PERFORMANCE ? "P-Core" : "E-Core";
    ...
    socket.onmessage = (me) => {
      try {
        ...
        systemStats.value = {
          ...
          netInterfaces: (s.io?.networks || []).map((n) => ({
            name: n.name || "",
            readSpeed: Number(n.recvBytes || 0),
            writeSpeed: Number(n.sentBytes || 0),
          })),
          diskDevices: (s.io?.disks || []).map((d) => ({
            name: d.name || "",
            readSpeed: Number(d.readBytes || 0),
            writeSpeed: Number(d.writeBytes || 0),
          })),
        ...
        const now = Date.now();
        statsHistory.value.cpu.push({
          time: now,
          value: systemStats.value.cpuTotal,
        });

        gpus.value.forEach((gpu) => {
          if (!statsHistory.value.gpus[gpu.index])
            statsHistory.value.gpus[gpu.index] = {
              util: [],
              mem: [],
              vram: [],
              power: [],
          ...
          gh.vram.push({
            time: now,
            value: Number(gpu.memUsed || 0) * 1024 * 1024,
          });
        ...
        systemStats.value.cpuCoresDetailed.forEach((core) => {
          if (!statsHistory.value.cores[core.index])
            statsHistory.value.cores[core.index] = [];
          statsHistory.value.cores[core.index].push({
            time: now,
            value: core.usage,
          });
          if (statsHistory.value.cores[core.index].length > 60)
            statsHistory.value.cores[core.index].shift();
        });

        systemStats.value.netInterfaces.forEach((iface) => {
          if (!statsHistory.value.netDevices[iface.name])
            statsHistory.value.netDevices[iface.name] = { recv: [], sent: [] };
        ...
        systemStats.value.diskDevices.forEach((disk) => {
          if (!statsHistory.value.diskDevices[disk.name])
            statsHistory.value.diskDevices[disk.name] = { read: [], write: [] };
        ...
        statsHistory.value.mem.push({
          time: now,
          value: systemStats.value.memPercent,
        });
        statsHistory.value.memUsed.push({
          time: now,
          value: systemStats.value.memUsed,
        });
        statsHistory.value.memCached.push({
          time: now,
          value: systemStats.value.memCached,
        });
        statsHistory.value.memBuffers.push({
          time: now,
          value: systemStats.value.memBuffers,
        });

        if (systemStats.value.swapTotal > 0)
          statsHistory.value.swapUsage.push({
            time: now,
            value: systemStats.value.swapUsed,
          });
        if (systemStats.value.zramTotal > 0)
          statsHistory.value.zramUsage.push({
            time: now,
            value: systemStats.value.zramUsed,
          });

        statsHistory.value.netRecv.push({
          time: now,
          value: systemStats.value.totalNetRecv,
        });
        statsHistory.value.netSent.push({
          time: now,
          value: systemStats.value.totalNetSent,
        });
        statsHistory.value.diskRead.push({
          time: now,
          value: systemStats.value.totalDiskRead,
        });
        statsHistory.value.diskWrite.push({
          time: now,
          value: systemStats.value.totalDiskWrite,
        });
        statsHistory.value.faults.push({
          time: now,
          value: systemStats.value.faults.pageFaultRate,
        });
        statsHistory.value.swapIn.push({
          time: now,
          value: systemStats.value.faults.swapInRate,
        });
        statsHistory.value.swapOut.push({
          time: now,
          value: systemStats.value.faults.swapOutRate,
        });
  ...
  const historyChartOptions = computed(() => {
    ...
    const isByteChart =
      ...
      t.includes("write") ||
      t.includes("activity") ||
      t.includes("memory usage") ||
      t.includes("swap usage") ||
      t.includes("zram usage") ||
      (t.includes("vram") && t.includes("usage"));
        ...
        labels: {
          ...
          formatter: (val: number) => {
            ...
            if (isByteChart) {
              if (!t.includes("%"))
                return (
                  formatBytesWithUnit(val) + (t.includes("usage") ? "" : "/s")
                );
            }
  ...
  const showProcessDetails = async (record: any) => {
    ...
    try {
      ...
      selectedProcessMaps.value =
        "Failed to fetch maps. Process might have exited.";
    } finally {
