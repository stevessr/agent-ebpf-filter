...
export function useSystemd() {
  ...
  const filteredSystemdServices = computed(() => {
    ...
    return systemdServices.value.filter(
      (s) =>
  ...
  const systemdColumns = [
    ...
    },
    {
      ...
      onFilter: (v: string, r: any) => r.active === v,
    },
  ...
  const fetchSystemdServices = async () => {
    ...
    try {
      const res = await axios.get(
        `/system/systemd?scope=${systemdScope.value}`,
      );
  ...
  const controlSystemdService = async (unit: string, action: string) => {
    try {
      await axios.post("/system/systemd/control", {
        unit,
        action,
        scope: systemdScope.value,
      });
      message.success(
        `${systemdScope.value.toUpperCase()} service ${unit} ${action} command sent`,
      );
      void fetchSystemdServices();
    } catch (err: any) {
      message.error(
        err?.response?.data?.error || `Failed to ${action} service`,
      );
    }
  };
  ...
  const fetchSystemdLogs = async (unit: string) => {
    ...
    try {
      const res = await axios.get(
        `/system/systemd/logs?unit=${unit}&lines=200&scope=${systemdScope.value}`,
      );
