...
function normalizeImportedAgentSightRecord(value: any, index: number): AgentSightEventRecord | null {
  ...
  if (value.Event || value.Envelope || value.Timestamp || value.event || value.envelope || value.timestamp || value.source) return value as AgentSightEventRecord;
...
function tlsCaptureEventToRecord(event: Record<string, any>, index = 0): AgentSightEventRecord {
  const timestamp = event.timestamp || Date.now() + index;
  const source = event.type === 'sse_message' ? 'sse_processor' : event.type === 'http_request' || event.type === 'http_response' ? 'http_parser' : 'ssl';
  return {
...
function systemStatsToRecords(stats: Record<string, any>, timestamp = Date.now()): AgentSightEventRecord[] {
  ...
  const ranked = processes
    .slice()
    .sort((a: any, b: any) => Math.max(numberValue(b.cpu), numberValue(b.mem)) - Math.max(numberValue(a.cpu), numberValue(a.mem)))
    .slice(0, AGENTSIGHT_SYSTEM_TOP_PROCESSES);

  const records: AgentSightEventRecord[] = ranked.map((process: any, index: number) => {
    const memPercent = numberValue(process.mem, 0);
    const memoryBytes = memoryTotal > 0 ? Math.round((memoryTotal * memPercent) / 100) : 0;
  ...
  if (stats.cpu || stats.memory) {
    records.unshift({
      ...
      data: {
        ...
        alert: numberValue(stats.cpu?.total, 0) >= 80 || numberValue(stats.memory?.percent, 0) >= 80,
      },
    });
  }
...
export function useAgentSightEvents() {
  ...
  const realRecords = computed(() => [...importedRecords.value, ...systemRecords.value, ...tlsRecords.value, ...liveRecords.value]);
  const rawRecords = computed(() => realRecords.value.length > 0 ? realRecords.value : sampleRecords.value);
  const events = computed<AgentSightEvent[]>(() => applyAgentSightLimit(normalizeAgentSightEvents(rawRecords.value), limit.value));
  ...
  const fetchEnvelopeEvents = async () => {
    ...
    try {
      ...
      message.error(error?.response?.data?.error || 'Failed to load AgentSight event envelopes');
    } finally {
      loading.value = false;
    }
  };
  ...
  const fetchTLSEvents = async () => {
    ...
    try {
      const response = await axios.get(`/tls-capture/recent?limit=${agentSightLimitQuery(limit.value)}`);
      const tls = Array.isArray(response.data?.events) ? response.data.events.map(tlsCaptureEventToRecord) : [];
      tlsRecords.value = tls;
    } catch (error: any) {
      if (error?.response?.status && error.response.status !== 404) {
        message.warning(error?.response?.data?.error || 'TLS capture history is not available');
      }
      tlsRecords.value = [];
    } finally {
  ...
  const mergeEnvelopeRecords = (incoming: Record<string, any>[]) => {
    ...
    const records = incoming.map(envelope => ({ Envelope: envelope, Timestamp: (envelope.timestamp_ns || envelope.timestampNs) ? Math.floor(Number(envelope.timestamp_ns || envelope.timestampNs) / 1_000_000) : Date.now() }));
    liveRecords.value = applyAgentSightLimit([...records.reverse(), ...liveRecords.value], limit.value);
  };
  ...
  const connectEnvelopeWebSocket = () => {
    ...
    socket.onclose = () => {
      ...
      if (shouldReconnect) envelopeReconnectTimer = setTimeout(connectEnvelopeWebSocket, 3000);
    };
    socket.onerror = () => {
      isEnvelopeConnected.value = false;
    };
  ...
  const connectTLSWebSocket = () => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        tlsRecords.value = applyAgentSightLimit([tlsCaptureEventToRecord(payload), ...tlsRecords.value], limit.value);
      } catch (error) {
    ...
    socket.onclose = () => {
      ...
      if (shouldReconnect) tlsReconnectTimer = setTimeout(connectTLSWebSocket, 3000);
    };
    socket.onerror = () => {
      isTLSConnected.value = false;
    };
  ...
  const connectSystemWebSocket = () => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        systemRecords.value = applyAgentSightLimit([...systemStatsToRecords(plain, timestamp), ...systemRecords.value], limit.value);
      } catch (error) {
    ...
    socket.onclose = () => {
      ...
      if (shouldReconnect) systemReconnectTimer = setTimeout(connectSystemWebSocket, 3000);
    };
    socket.onerror = () => {
      isSystemConnected.value = false;
    };
  ...
  const loadSampleTrace = async () => {
    ...
    try {
      const response = await axios.get(AGENTSIGHT_SAMPLE_TRACE_URL, { responseType: 'text' });
  ...
  const autoLoadSampleIfEmpty = async () => {
    if (sampleAttempted.value || realRecords.value.length > 0 || sampleRecords.value.length > 0) return;
  ...
  const visibleEvents = computed(() => {
    ...
    return events.value.filter(event => {
      ...
      if (f.comm && !event.comm.toLowerCase().includes(f.comm.toLowerCase())) return false;
      ...
      if (f.searchTerm) {
        ...
        if (![event.title, event.source, event.eventType, event.comm, event.id, JSON.stringify(event.data)].some(value => String(value || '').toLowerCase().includes(q))) return false;
      }
      return true;
    });
  });

  const visibleProcessedEvents = computed(() => processAgentSightEvents(visibleEvents.value));
  ...
  const metrics = computed(() => {
    ...
    return {
      ...
      http: list.filter(event => event.source === 'http_parser' || event.eventType.includes('HTTP')).length,
      sse: list.filter(event => event.source === 'sse_processor' || event.eventType.includes('SSE')).length,
      sanitized: list.filter(event => event.redactionState === 'sanitized').length,
      alerts: list.filter(event => event.source === 'policy' || event.eventType.includes('ALERT')).length,
  ...
  const exportVisibleJSON = () => {
    downloadText(`agentsight-events-${Date.now()}.json`, JSON.stringify(visibleEvents.value, null, 2), 'application/json;charset=utf-8');
  };

  const exportVisibleJSONL = () => {
    downloadText(`agentsight-events-${Date.now()}.jsonl`, visibleEvents.value.map(event => JSON.stringify(event.raw ?? event)).join('\n') + '\n', 'application/x-ndjson;charset=utf-8');
  };

  const exportVisibleCSV = () => {
    const rows = [
      ['timestamp', 'source', 'event_type', 'pid', 'ppid', 'comm', 'trace_id', 'redaction_state', 'summary'],
      ...visibleEvents.value.map(event => [event.timestamp, event.source, event.eventType, event.pid, event.ppid || '', event.comm, event.traceId, event.redactionState, event.title]),
    ];
    downloadText(`agentsight-events-${Date.now()}.csv`, rows.map(row => row.map(csvCell).join(',')).join('\n') + '\n', 'text/csv;charset=utf-8');
  };
