...
```
关联维度：
1. 网络事件 + 文件访问（读取配置 → 连接服务器）
2. 网络事件 + 进程创建（fork 后立即连接）
3. DNS 查询 + TCP 连接（域名解析 → IP 连接）
...
```
调用 tail_events 获取最近 500 个事件
过滤 eventType = "connect"
按 pid 分组，统计每个 pid 连接的不同端口数
如果某个 pid 在短时间内连接了 >10 个不同端口 → 可疑
```
